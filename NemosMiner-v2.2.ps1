param(
    ...
    [String]$UserName, 
    [Parameter(Mandatory=$false)]
    [String]$WorkerName = "ID=NemosMiner-v2.2", 
    [Parameter(Mandatory=$false)]
    [Int]$API_ID = 0, 
    [Parameter(Mandatory=$false)]
    [String]$API_Key = "", 
...
$WorkerNameDonate = "NemosMiner-v2.2"
$WalletBackup = $Wallet
...
while($true)
{
    ...
    $Currency | Where-Object {$NewRates.$_} | ForEach-Object {$Rates | Add-Member $_ ([Double]$Rates.$_) -Force}
    ...
    if($AllPools.Count -eq 0){"No Pools!" | Out-Host; sleep 10; continue}
