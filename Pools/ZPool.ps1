...
If ($DivisorMultiplier -and $PriceField -and $Wallet) { 
    ...
    Try { 
        $Request = Get-Content "$(Split-Path -Parent (Get-Item $MyInvocation.MyCommand.Path).Directory)\Data\BrainData_$((Get-Item $MyInvocation.MyCommand.Path).BaseName).json" -ErrorAction Stop | ConvertFrom-Json
    }
    Catch { Return }
