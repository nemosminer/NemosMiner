...
If ($Config.Transcript) { Start-Transcript -Path ".\Debug\$((Get-Item $MyInvocation.MyCommand.Path).Name)-Transcript_$(Get-Date -Format "yyyy-MM-dd_HH-mm-ss").log" }

$Counter = 0
"" > ".\Debug\$((Get-Item $MyInvocation.MyCommand.Path).Name)_MemUsage.log"
...
Do { 
    ...
    Try { 
        ...
        # If (-not ($Counter % 100)) { 
            ...
            "$(Get-Date -Format "yyyy-MM-dd HH:mm:ss"): Core 'Loop': {0:n2} MBytes / $(If (($MemUsage - $MemUsageLastCycle) -gt 0) { "+" } ){1:n0} kBytes [Cycle $Counter / Uptime: $((([DateTime]::Now).ToUniversalTime() - $Variables.ScriptStartTime).ToString('hh\:mm\:ss'))]" -f ($MemUsage / 1MB), (($MemUsage - $MemUsageLastCycle) / 1kB) >> ".\Debug\$((Get-Item $MyInvocation.MyCommand.Path).Name)_MemUsage.log"
        # }
