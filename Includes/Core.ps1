...
Do { 
    Try {
        ...
        If ([String]$Variables.EnabledDevices.Name -ne [String]($Variables.Devices | Where-Object { $_.State -eq [DeviceState]::Enabled }).Name) { $Variables.EnabledDevices = [Device[]]@(($Variables.Devices | Where-Object { $_.State -eq [DeviceState]::Enabled } ).Clone()) }
        If ($Variables.EnabledDevices) { 
            ...
            If ([String]($Config.PoolName | Sort-Object) -ne [String]($Variables.PoolName | Sort-Object) -or -not $Variables.Miners -or -not $Variables.Timer -or $Variables.Timer.AddSeconds([Int]($Config.Interval / 2)) -lt (Get-Date).ToUniversalTime() -or (Compare-Object @($Config.ExtraCurrencies | Select-Object) @($Variables.AllCurrencies | Select-Object) | Where-Object SideIndicator -eq "<=")) { 
                ...
                $PoolBaseNames = Get-PoolBaseName $Variables.PoolName
                # Pools config has changed, stop deconfigured brains
                If ([String]($Config.PoolNamne | Sort-Object) -ne [String]($Variables.PoolName | Sort-Object)) { 
                    Stop-Brain @($Variables.Brains.Keys | Where-Object { $_ -notin $PoolBaseNames })
                }
                ...
                If ($Config.WebGUI -eq $true) { Start-APIServer } Else { Stop-APIServer }
                ...
                ForEach ($Miner in $Variables.MinersBest_Combo) { 
                    # Reduce data to MinDataSample * 5
                    $Miner.Data = @($Miner.Data | Select-Object -Last ($Miner.MinDataSample * 5))
                        ...
                        If ($Miner.GetStatus() -eq [MinerStatus]::Running) { 
                            ...
                            If ($Config.BadShareRatioThreshold -gt 0) { 
                                $Miner.WorkersRunning.Pool.Algorithm | ForEach-Object { 
                                    $LatestMinerData = ($Miner.Data | Select-Object -Last 1).Shares
                                    If ($LatestMinerData.$_ -and $LatestMinerData.$_[1] -gt 0 -and $LatestMinerData.$_[2] -gt [Int](1 / $Config.BadShareRatioThreshold) -and $LatestMinerData.$_[1] / $LatestMinerData.$_[2] -gt $Config.BadShareRatioThreshold) { 
                                        $Miner.StatusMessage = "Miner '$($Miner.Name) $($Miner.Info)' stopped. Reasons: Too many bad shares (Shares Total = $($LatestMinerData.$_[2]), Rejected = $($LatestMinerData.$_[1]))."
                    ...
                    If ($Miner.Activated -gt 0) { 
                        ...
                        If ($Miner.Data.Count -ge $Miner.MinDataSample -or $Miner.Activated -gt $Variables.WatchdogCount) { 
                            ...
                            ForEach ($Worker in $Miner.WorkersRunning) { 
                                ...
                                $Factor = 1
                                $LatestMinerData = ($Miner.Data | Select-Object -Last 1).Shares
                                If ($Miner.Data.Count -gt $Miner.MinDataSample -and -not $Miner.Benchmark -and $Config.SubtractBadShares -and $LatestMinerData.$Algorithm -gt 0) { # Need $Miner.MinDataSample shares before adjusting hashrate
                                    $Factor = (1 - $LatestMinerData.$Algorithm[1] / $LatestMinerData.$Algorithm[2])
                                ...
                                If ($Stat.Updated -gt $Miner.StatStart) { 
                                    Write-Message -Level Info "Saved hashrate for '$($Stat_Name -replace '_Hashrate$')': $(($Miner_Hashrates.$Algorithm | ConvertTo-Hash) -replace ' ')$(If ($Factor -le 0.999) { " (adjusted by factor $($Factor.ToString('N3')) [Shares total: $($LatestMinerData.$Algorithm[2]), rejected: $($LatestMinerData.$Algorithm[1])])" })$(If ($Stat.Duration -eq $Stat_Span) { " [Benchmark done]" })."
                        ...
                        # We don't want to store power usage if we have less than $MinDataSample
                        If ($Miner.Data.Count -ge $Miner.MinDataSample -and $Miner.ReadPowerUsage -and ($Stat.Updated -gt $Miner.StatStart -or $Miner.Activated -gt $Variables.WatchdogCount)) { 
                ...
                If ($Variables.PoolName) { 
                    If ($Variables.Brains.Keys | Where-Object { $Variables.Brains.$_.StartTime -gt $Variables.Timer.AddSeconds(- $Config.Interval) }) { 
                        ...
                        If ($Variables.Brains.Keys) {
                            $Variables.PoolTimeout = $ExtendLoop = 60
                            $Variables.Summary = "Loading initial pool data from '$($Variables.PoolName -join ', ')'.<br>This may take up to $($ExtendLoop) seconds..."
                            Write-Message -Level Info ($Variables.Summary -replace "<br>", " ")
                            Do { 
                                Start-Sleep -Seconds 1
                                $ExtendLoop --
                            } While ($Variables.BrainData.Count -lt $Variables.Brains.Keys.Count -and $EndLoop -gt 0)
                    ...
                    $NewPools = [Pool[]]@()
                    $Variables.PoolName | ForEach-Object -TimeoutSeconds $Variables.PoolTimeout -Parallel { 
                        $Config = $Using:Config
                        $PoolsConfig = $Using:PoolsConfig
                        $PoolBaseName = $_ -replace "24hr$|Coins$|Plus$"
                        $PoolConfig = $PoolsConfig.$PoolBaseName
                        $Variables = $Using:Variables
                        & ".\Pools\$PoolBaseName.ps1" -Config $Config -PoolConfig $PoolConfig -PoolVariant $_ -Variables $Variables
                        [System.GC]::Collect()
                    } | ForEach-Object { $NewPools += $_ -as [Pool] }
                    ...
                    If ($ComparePools = @(Compare-Object -PassThru $NewPools $Pools -Property Name, Algorithm -IncludeEqual)) { 
                        ...
                        # Sort best pools
                        $SortedAvailablePools = $Pools | Where-Object Available -EQ $true | Sort-Object { $_.Name -notin $Variables.PoolNameToKeepBalancesAlive }, { - $_.StablePrice * $_.Accuracy }
            ...
            If ($Miners | Where-Object Available -EQ $true) { 
                ...
                Else { 
                    ...
                    $Variables.MinersMostProfitable = @($Miners | Where-Object Available -EQ $true | Group-Object { [String]$_.DeviceNames }, { [String]$_.Algorithms } | ForEach-Object { 
                        $_.Group | Sort-Object -Descending -Property Benchmark, MeasurePowerUsage, KeepRunning, Prioritize, "$($SortBy)_Bias", Activated, @{ Expression = { $_.WarmupTimes[1] }; Descending = $true }, @{ Expression = { $_.Name }; Descending = $false },  @{ Expression = { [String]($_.Algorithms) }; Descending = $false } | Select-Object -First 1 | ForEach-Object { 
                            $_.MostProfitable = $true; $_
                        }
                    })
                    $Variables.MinersBest = @($Variables.MinersMostProfitable | Group-Object { [String]$_.DeviceNames } | ForEach-Object { 
                        $_.Group | Sort-Object -Descending -Property Benchmark, MeasurePowerUsage, KeepRunning, Prioritize, "$($SortBy)_Bias", Activated, @{ Expression = { $_.WarmupTimes[1] }; Descending = $true }, @{ Expression = { $_.Name }; Descending = $false } | Select-Object -First 1
                    })
                    ...
                    # $Variables.MinersBest_Combo = @(($Variables.MinersBest_Combos | Sort-Object -Descending { @($_.Combination | Where-Object Benchmark).Count }, { @($_.Combination | Where-Object MeasurePowerUsage).Count }, { ($_.Combination | Measure-Object "$($SortBy)_Bias" -Sum).Sum } | Select-Object -First 1).Combination)
        ...
        $Miners = $Miners | Where-Object { $_.Workers[0].Pool.BaseName -in $PoolBaseNames -and ($_.Workers.Count -eq 1 -or $_.Workers[1].Pool.BaseName -in $PoolBaseNames) }
        ...
        Do { 
            ...
            ForEach ($Miner in $Variables.RunningMiners) { 
                ...
                ElseIf ($Miner.DataReaderJob.HasMoreData) { 
                    ...
                    $Miner.Data += $Samples = @($Miner.DataReaderJob | Receive-Job | Select-Object -Property Date, Hashrate, Shares, PowerUsage)

                    If ($Sample = $Samples | Select-Object -Last 1) { 
                        $Miner.Hashrates_Live = $Sample.Hashrate.PSObject.Properties.Value
                        If ($Sample.Date -lt $Miner.BeginTime.AddSeconds($Miner.WarmupTimes[1])) { 
                            ...
                            $Miner.Data = @($Miner.Data | Where-Object Date -GE $Miner.BeginTime.AddSeconds($Miner.WarmupTimes[1]))
                        }
                        Else { 
