...
Function Core-Loop { 
    ...
    If (-not $Variables.Timer -or $Variables.Timer.AddSeconds([Int]($Config.Interval / 2)) -lt (Get-Date).ToUniversalTime()) { 
        ...
        If ($Config.Donate -gt 0) { 
            ...
            If ((Get-Date) -ge $Variables.DonateStart -and $null -eq $Variables.DonateEnd) { 
                ...
                $Variables.DonatePoolNames = @(Get-PoolName (Get-ChildItem .\Pools\*.ps1 -File).BaseName | Sort-Object -Unique)
        ...
        Stop-BrainJob @($Variables.BrainJobs.Keys | Where-Object { $_ -notin @(Get-PoolName $PoolNames) })
        ...
        $ComparePools | Select-Object | Where-Object { -not $_.BaseName } | ForEach-Object { $_ | Add-Member BaseName (Get-PoolName $_.Name) -Force }
        ...
        $Pools | Select-Object | ForEach-Object { 
            ...
            $_.Price_Bias = $_.Price * (1 - $_.Accuracy)
        }
        ...
        If ($Variables.CycleStarts.Count -ge $Config.SyncWindow) { 
            ...
            $Pools | ForEach-Object { 
                ...
                ElseIf ($_.Updated -lt $Variables.CycleStarts[0]) { $_.Price_Bias = $_.Price * (1 - $_.Accuracy) * [Math]::Pow(0.9, ($Variables.CycleStarts[0] - $_.Updated).TotalMinutes) }
            }
        ...
        # SSL filter
        If ($Config.SSL -ne "Preferred") { $Pools | Where-Object { $_.SSL -ne [Boolean]$Config.SSL } | ForEach-Object { $_.Reason += "Config item SSL -ne $([Boolean]$Config.SSL)" } }
        # Min accuracy exceeded
        $Pools | Where-Object Accuracy -GT (1 - $Config.MinAccuracy) | ForEach-Object { $_.Reason += "MinAccuracy ($($Config.MinAccuracy * 100)%) exceeded" }
        ...
        # Pool not configured
        $Pools | Where-Object { $_.Name -notin $PoolNames } | ForEach-Object { $_.Reason += "Pool not configured" }
        # Pool price 0
        ...
        # Region exclusions
        $Pools | Where-Object { $Config.Pools.($_.BaseName).ExcludeRegion -and (Compare-Object @($Config.Pools.$($_.BaseName).ExcludeRegion | Select-Object) @($_.Region) -IncludeEqual -ExcludeDifferent) } | ForEach-Object { $_.Reason += "Region excluded (in $($_.BaseName) pool config)" }
        # MinWorkers
        $Pools | Where-Object { $null -ne $_.Workers -and $_.Workers -lt $Variables.PoolsConfigData.($_.BaseName).MinWorker } | ForEach-Object { $_.Reason += "Not enough workers at pool (MinWorker ``$($Config.PoolsConfig.($_.BaseName).MinWorker)`` in $($_.BaseName) pool config)" }
        $Pools | Where-Object { $null -ne $_.Workers -and $_.Workers -lt $Config.MinWorker -and $null -eq $Variables.PoolsConfigData.($_.BaseName).MinWorker } | ForEach-Object { $_.Reason += "Not enough workers at pool (MinWorker ``$($Config.MinWorker)`` in generic config)" }
    ...
    If ($Config.Watchdog -and $Variables.WatchdogTimers) { 
        $Pools | Where-Object { -not $_.Reason } | Group-Object -Property Name | ForEach-Object { 
            # Suspend pool if more than half of all available miners executed within the last 24hrs have failed $WatchdogCount times
            $PoolName = $_.Name
            $WatchdogCount = ($Variables.WatchdogCount, ($Variables.WatchdogCount * ($Variables.Miners | Where-Object { $_.Workers.Pool.Name -eq $PoolName } | Where-Object { $_.StatStart -gt $Variables.Timer.AddDays(-1) } ).count) | Measure-Object -Maximum).Maximum
            If (@($Variables.WatchdogTimers | Where-Object PoolName -EQ $PoolName | Where-Object Kicked -LE $Variables.Timer).Count -gt $WatchdogCount) { 
                $_.Group | ForEach-Object { $_.Reason += "Pool (all algorithms & regions) suspended by watchdog" }
                Write-Message -Level Warn "Pool '$($_.Name) (all algorithms & regions)' is suspended by watchdog until $((($Variables.WatchdogTimers | Where-Object PoolName -EQ $_.Name | Where-Object Kicked -LT $Variables.Timer).Kicked | Sort-Object | Select-Object -First 1).AddSeconds($Variables.WatchdogReset).ToLocalTime().ToString("T"))."
            }
        }
        $Pools | Where-Object { -not $_.Reason } | Where-Object Reason -notlike "* suspended by watchdog" | Group-Object -Property Name, Region | ForEach-Object { 
            # Suspend pool if more than half of all available miners (region) executed within the last 24hrs have failed $WatchdogCount times
            $PoolName = $_.Group[0].Name
            $PoolRegion = $_.Group[0].Region
            $WatchdogCount = ($Variables.WatchdogCount, ($Variables.WatchdogCount * ($Variables.Miners | Where-Object Available | Where-Object { $_.Workers.Pool.Name -eq $PoolName } | Where-Object { $_.Workers.Pool.Region -eq $PoolRegion } | Where-Object { $_.StatStart -gt $Variables.Timer.AddDays(-1) } | Select-Object -Property Name -Unique).count / 2) | Measure-Object -Maximum).Maximum
            If (@($Variables.WatchdogTimers | Where-Object PoolName -EQ $PoolName | Where-Object PoolRegion -EQ $PoolRegion | Where-Object Kicked -LE $Variables.Timer).Count -gt $WatchdogCount) { 
                $_.Group | ForEach-Object { $_.Reason += "Pool (Region: $PoolRegion) suspended by watchdog" }
                Write-Message -Level Warn "Pool '$PoolName ($PoolRegion)' is suspended by watchdog until $((($Variables.WatchdogTimers | Where-Object PoolName -EQ $PoolName | Where-Object PoolRegion -EQ $PoolRegion | Where-Object Kicked -LT $Variables.Timer).Kicked | Sort-Object | Select-Object -First 1).AddSeconds($Variables.WatchdogReset).ToLocalTime().ToString("T"))."
            }
        }
        $Pools | Where-Object { -not $_.Reason } | Where-Object Reason -notlike "* suspended by watchdog" | Group-Object -Property Name, Algorithm, Region | ForEach-Object { 
            # Suspend algorithm@pool if more than half of all available miners (algorithm and region) executed within the last 24hrs have failed $WatchdogCount times
            $Algorithm = $_.Group[0].Algorithm
            $PoolName = $_.Group[0].Name
            $PoolRegion = $_.Group[0].Region
            $WatchdogCount = ($Variables.WatchdogCount, ($Variables.WatchdogCount * ($Variables.Miners | Where-Object Available | Where-Object { $_.Workers.Pool.Algorithm -eq $Algorithm } | Where-Object { $_.Workers.Pool.Name -eq $PoolName } | Where-Object { $_.Workers.Pool.Region -eq $PoolRegion } | Where-Object { $_.StatStart -gt $Variables.Timer.AddDays(-1) } | Select-Object -Property Name -Unique).count / 2) | Measure-Object -Maximum).Maximum
            If (@($Variables.WatchdogTimers | Where-Object Algorithm -EQ $Algorithm | Where-Object PoolName -EQ $PoolName | Where-Object PoolRegion -EQ $PoolRegion | Where-Object Kicked -LE $Variables.Timer).Count -gt $WatchdogCount) { 
                $_.Group | ForEach-Object { $_.Reason += "Pool (Algorithm: $Algorithm, Region: $PoolRegion)) suspended by watchdog" }
                Write-Message -Level Warn "Pool '$PoolName (Algorithm: $Algorithm, Region: $PoolRegion)' is suspended by watchdog until $((($Variables.WatchdogTimers | Where-Object Algorithm -EQ $Algorithm | Where-Object PoolName -EQ $PoolName | Where-Object PoolRegion -EQ $PoolRegion | Where-Object Kicked -LT $Variables.Timer).Kicked | Sort-Object | Select-Object -First 1).AddSeconds($Variables.WatchdogReset).ToLocalTime().ToString("T"))."
            }
        }
        Remove-Variable Algorithm, PoolName, PoolRegion, WatchdogCount, PoolsToSuspend -ErrorAction Ignore
    }
    ...
    # Sort best pools
    [Pool[]]$AvailablePools = $Pools | Where-Object Available | Sort-Object { - $_.StablePrice * (1 - $_.Accuracy) }, { $_.SSL -ne $Config.SSL }, { $Variables.Regions.($Config.Region).IndexOf($_.Region) }
    ($AvailablePools.Algorithm | Sort-Object -Unique) | ForEach-Object { 
        $AvailablePools | Where-Object Algorithm -EQ $_ | Select-Object -First 1 | ForEach-Object { $_.Best = $true }
    }
    ...
    If ($AvailablePools) { 
        ...
        $NewMiners_Jobs = @(
            ...
            # Get-ChildItemContent ".\Miners_Dev" -Parameters @{ Pools = $PoolsPrimaryAlgorithm; PoolsSecondaryAlgorithm = $PoolsSecondaryAlgorithm; Config = $Config; Devices = $EnabledDevices } -Threaded -Priority $(If ($Miners | Where-Object { $_.Status -eq [MinerStatus]::Running } | Where-Object { $_.DeviceName -like "CPU#*" }) { "Normal" })
        )
    }
    ElseIf ($Variables.Miners) { $Variables.Miners | ForEach-Object { $_.Available = $false } }
    ...
    ForEach ($Miner in ($Variables.Miners | Where-Object Best)) { 
        ...
        If ($Miner.Data.Count) { 
            ...
            ForEach ($Algorithm in $Miner.Algorithm) { 
                $CollectedHashRate = $Miner.CollectHashRate($Algorithm, ($Miner.Benchmark -or $Miner.Data.Count -lt $Miner.MinDataSamples))
            ...
            If ($Miner.ReadPowerUsage) { 
                ...
                $CollectedPowerUsage = $Miner.CollectPowerUsage($Miner.MeasunrePowerUsage -or $Miner.Data.Count -lt $Miner.MinDataSamples)
        ...
        If ($Miner.Activated -gt 0) { 
            ...
            If ($Miner.Data.Count -ge $Miner.MinDataSamples -or $Miner.Activated -ge 3) { 
    ...
    If ($AvailablePools) { 
        ...
        $CompareMiners = [Miner[]](Compare-Object -PassThru @($Miners | Select-Object) @($NewMiners | Select-Object) -Property Name, Algorithm, Arguments -IncludeEqual)
        ...
        If ($Config.Watchdog -and $Variables.WatchdogTimers) { 
            $Miners | Where-Object Available -EQ $true | Group-Object -Property { "$($_.BaseName)-$($_.Version)" } | ForEach-Object { 
                ...
                If (@($Variables.WatchdogTimers | Where-Object MinerBaseName -EQ $_.BaseName | Where-Object MinerVersion -EQ $_.Version | Where-Object Kicked -LT $Variables.Timer).Count -gt $WatchdogMinerCount) { 
                    $_.Group | ForEach-Object { $_.Reason += "Miner (all algorithms) suspended by watchdog" }
                    Write-Message -Level Warn "Miner '$($_.Group[0].BaseName)-$($_.Group[0].Version) (all algorithms)' is suspended by watchdog until $((($Variables.WatchdogTimers | Where-Object MinerBaseName -EQ $_.Group[0].BaseName | Where-Object MinerVersion -EQ $_.Group[0].Version | Where-Object Kicked -LT $Variables.Timer).Kicked | Sort-Object | Select-Object -First 1).AddSeconds($Variables.WatchdogReset).ToLocalTime().ToString("T"))."
                }
            }
            $Miners | Where-Object Available -EQ $true | Where-Object Reason -notlike "* suspended by watchdog" | Where-Object { @($Variables.WatchdogTimers | Where-Object MinerName -EQ $_.Name | Where-Object DeviceName -EQ $_.DeviceName | Where-Object Algorithm -EQ $_.Algorithm | Where-Object Kicked -LT $Variables.Timer).Count -ge $Variables.WatchdogCount } | ForEach-Object { 
                $_.Reason += "Miner (Algorithm: $($_.Algorithm)) suspended by watchdog"
                Write-Message -Level Warn "Miner '$($_.Name) (Algorithm: $($_.Algorithm))' is suspended by watchdog until $((($Variables.WatchdogTimers | Where-Object MinerName -EQ $_.Name | Where-Object DeviceName -EQ $_.DeviceName | Where-Object Algorithm -EQ $_.Algorithm | Where-Object Kicked -LT $Variables.Timer).Kicked | Sort-Object | Select-Object -First 1).AddSeconds($Variables.WatchdogReset).ToLocalTime().ToString("T"))."
            }
            $Miners | Where-Object Reason -like "* suspended by watchdog" | ForEach-Object { 
                $_.Data = @() # Clear data, it may be incorrect because of miner problem
            }
        }

        $Miner | Where-Object Reason | ForEach-Object { $_.Available = $false }
    ...
    ForEach ($Miner in @((@($Miners | Where-Object { $_.Status -eq [MinerStatus]::Running } | Where-Object { $_.Best -eq $false -or $_.Restart -eq $true} ) + @($CompareMiners | Where-Object { $_.SideIndicator -EQ "<=" -and $_.Status -eq [MinerStatus]::Running } <# miner object is gone #> )))) { 
        If ($Miner.GetStatus() -eq [MinerStatus]::Running -and $Miner.Data.Count -ge $Miner.MinDataSamples) { 
            ForEach ($Worker in $Miner.WorkersRunning) { 
    ...
    If (-not $AvailablePools) { 
        $Variables.Miners = $null
        Remove-Variable AvailablePools
        Write-Message -Level Warn "No pools available - retrying in 10 seconds..."
        Start-Sleep -Seconds 10
