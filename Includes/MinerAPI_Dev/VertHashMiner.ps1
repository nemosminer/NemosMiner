...
class VertHashMiner : Miner { 
    [Object]GetMinerData () { 
        $LogFile = (Get-ChildItem -Path (Split-Path $this.Path) -Filter "*.txt") | Sort-Object CreationTime | Select-Object -Last 1
        $Samples = @()
        ...
        Get-Content $LogFile | ForEach-Object { 
            ...
            If ($TimeStamp = ([DateTime](($Line_Simple -split ']  ' | Select-Object -First 1) -replace '\[' -replace '\]  ')).ToUniversalTime()) { 
                ...
                If ($Line_Data -match '^GPU[0-9]+\d\.\d. ( |k|m|g)H/s') { 
                ...
                ElseIf ($Line_Data -match '^(accepted|rejected): [0-9]+/[0-9]+ \([0-9]+\.[0-9]+%\), [0-9]+\.[0-9]+ ( |k|m|g)H/s') { 
