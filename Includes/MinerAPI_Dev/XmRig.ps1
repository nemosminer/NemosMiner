    ...
    CreateConfigFiles() { 
        ...
        Try { 
            ...
            If ($Parameters.ConfigFile.Content.threads) { 
                ...
                $ThreadsConfig = Get-Content $ThreadsConfigFileErrorAction Ignore | ConvertFrom-JsonErrorAction Ignore
                If ($ThreadsConfig.Count -lt 1) { 
                    If (Test-Path -Path "$(Split-Path $this.Path)\$($this.Algorithms[0] | Select-Object -First 1)-*.json" -PathType Leaf) { 
                        ...
                        Remove-Item "$(Split-Path $this.Path)\ThreadsConfig-$($this.Algorithms[0] | Select-Object -First 1)-*.json" -ForceErrorAction Ignore
                    }
                    ...
                    If ($this.Process) { 
                        ...
                        For ($WaitForThreadsConfig = 0; $WaitForThreadsConfig -le 60; $WaitForThreadsConfig ++) { 
                            If ($ThreadsConfig = @(Get-Content $ThreadsConfigFileErrorAction Ignore | ConvertFrom-JsonErrorAction Ignore).threads) { 
                                ...
                                Else { 
                                    ConvertTo-Json -InputObject @($ThreadsConfig | Select-Object -Unique) -Depth 10 | Out-File -FilePath $ThreadsConfigFile -Force -EncodingErrorAction Ignore
                                }
                                Break
                            }
                ...
                If (-not ((Get-Content $ConfigFileErrorAction Ignore | ConvertFrom-JsonErrorAction Ignore).threads)) { 
