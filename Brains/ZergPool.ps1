...
While ($Config) { 
    ...
    Do {
        Try { 
          $CurrenciesData = Invoke-RestMethod -Uri $Config.PoolCurrenciesUri -Headers @{ "Cache-Control" = "no-cache" } -SkipCertificateCheck
            ...
            Else{ 
                $AlgoData = Invoke-RestMethod -Uri $Config.PoolStatusUri -Headers @{ "Cache-Control" = "no-cache" } -SkipCertificateCheck
            }
            $APICallFails = 0
        }
        Catch { 
