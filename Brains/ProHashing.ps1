...
While ($Config) { 
    ...
    Do {
        Try { 
            If (-not $AlgoData) { $AlgoData = (Invoke-RestMethod -Uri $Config.PoolstatusUri -Headers @{ "Cache-Control" = "no-cache" } -SkipCertificateCheck).data }
            If (-not $CurrenciesData) { $CurrenciesData = (Invoke-RestMethod -Uri $Config.PoolCurrenciesUri -Headers @{ "Cache-Control" = "no-cache" } -SkipCertificateCheck).data }
            $APICallFails = 0
        }
        Catch { 
            $APICallFails++
