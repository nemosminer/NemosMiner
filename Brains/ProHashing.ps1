...
While ($BrainConfig = $Config.PoolsConfig.$BrainName.BrainConfig) { 
    Try { 
        ...
        Do {
            Try { 
                If (-not $AlgoData) { 
                    $AlgoData = (Invoke-RestMethod -Uri $BrainConfig.PoolStatusUri -Headers $Headers -UserAgent $UserAgent -SkipCertificateCheck -TimeoutSec $BrainConfig.PoolAPITimeout).data
                }
                If (-not $CurrenciesData) { 
                    $CurrenciesData = (Invoke-RestMethod -Uri $BrainConfig.PoolCurrenciesUri -Headers $Headers -UserAgent $UserAgent -SkipCertificateCheck -TimeoutSec $BrainConfig.PoolAPITimeout).data
                }
                $APICallFails = 0
            }
            Catch { 
