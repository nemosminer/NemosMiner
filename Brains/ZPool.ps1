...
While ($BrainConfig) { 
    $SampleSizeMinutes = $BrainConfig.SampleSizeMinutes
    $SampleHalfPower = $BrainConfig.SampleHalfPower
    $Interval = $BrainConfig.Interval
    $TransferFile = "c:\Users\Stephan\Desktop\NemosMiner\Debug\($Name).json"#$BrainConfig.TransferFile
    $PoolStatusUri = $BrainConfig.PoolstatusUri
    $PoolCurrenciesUri = $BrainConfig.PoolsCurrenciesUri
    $PerAPIFailPercentPenalty = $BrainConfig.PerAPIFailPercentPenalty
    $AllowedAPIFailureCount = $BrainConfig.AllowedAPIFailureCount
    $UseFullTrust = $BrainConfig.UseFullTrust
    ...
    If ($AlgoData -and $CurrenciesData) { 
        ...
        # $Variables.BrainData | Add-Member $Name ($AlgoData | ConvertTo-Json).replace("NaN", 0)
