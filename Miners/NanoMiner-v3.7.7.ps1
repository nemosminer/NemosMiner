...
#   [PSCustomObject]@{ Algorithm = "EthashLowMem";  Type = "AMD"; Fee = 0.01;  MinMemGiB = $MinerPools[0].EthashLowMem.DAGSizeGiB + 1.08; Minerset = 2; Tuning = " -coreClocks +20 -memClocks +100 -memTweak 2"; WarmupTimes = @(45, 60); ExcludePool = @();                                           ExcludeGPUArchitecture = @(); Arguments = " -algo Ethash" } # PhoenixMiner-v6.2c is fastest
    ...
    [PSCustomObject]@{ Algorithm = "KawPow";        Type = "AMD"; Fee = 0.02;  MinMemGiB = $MinerPools[0].KawPow.DAGSizeGiB + 1.08;       Minerset = 2; Tuning = " -coreClocks +20 -memClocks +100 -memTweak 2"; WarmupTimes = @(45, 0);  ExcludePool = @("HashCryptos", "MiningDutch", "ProHashing"); ExcludeGPUArchitecture = @(); Arguments = " -algo KawPow" } # TeamRedMiner-v0.10.10 is fastest
