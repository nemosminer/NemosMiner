...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "KawPoW";               Type = "AMD"; MinMemGB = 3;    MinerSet = 2; Arguments = " --algo kawpow" } # NBMiner-v36.0 is fastest, but has 2% miner fee
        ...
        If ($SelectedDevices = @($Devices | Where-Object Type -EQ $_.Type | Where-Object Model -EQ $_.Model)) { 
            ...
            $AlgorithmDefinitions | Where-Object Type -EQ $_.Type | ForEach-Object {
                ...
                If ($Miner_Devices = @($SelectedDevices | Where-Object { $_.Type -eq "CPU" -or ($_.OpenCL.GlobalMemSize / 1GB) -ge $MinMemGB })) { 
                    ...
                    If ($_.Type -eq "AMD") { $Arguments += " --no-cpu --opencl --opencl-devices=$(($Miner_Devices | Sort-Object $DeviceEnumerator | ForEach-Object { '{0:x}' -f $_.$DeviceEnumerator }) -join ',')" }
                    If ($_.Type -eq "CPU") { $Arguments += " --threads=$($Miner_Devices.CIM.NumberOfLogicalProcessors -1)" }
                    If ($_.Type -eq "NVIDIA") { $Arguments += " --no-cpu --cuda --cuda-devices=$(($Miner_Devices | Sort-Object $DeviceEnumerator | ForEach-Object { '{0:x}' -f $_.$DeviceEnumerator }) -join ',')" }
