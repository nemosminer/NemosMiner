...
#    [PSCustomObject]@{ Algorithm = @("Ethash", "Heavyhash");       Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Ethash.DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @();        ExcludePool = @()       ; Arguments = " --algorithm ethash --algorithm heavyhash --gpu-auto-tune 2" }
     [PSCustomObject]@{ Algorithm = @("EthashLowMem");              Type = "AMD"; Fee = @(0.0065);       MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @();        ExcludePool = @()       ; Arguments = " --algorithm ethash" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
#    [PSCustomObject]@{ Algorithm = @("EthashLowMem", "HeavyHash"); Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @();        ExcludePool = @()       ; Arguments = " --algorithm ethash --algorithm heavyhash --gpu-auto-tune 2" }
     ...
     [PSCustomObject]@{ Algorithm = @("Kaspa");                     Type = "AMD"; Fee = @(0.0085);       MinMemGB = 1;                                     MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 30); ExcludeGPUArchitecture = @();        ExcludePool = @()       ; Arguments = " --algorithm kaspa" }
