...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "CryptonightCcx";       Type = "AMD"; MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 2; WarmupTimes = @(45, 0); Arguments = " --algo cn/ccx" } # SRBMinerMulti-v1.0.2 is fastest, but has 0.85% miner fee
...
#   [PSCustomObject]@{ Algorithm = "CryptonightR";         Type = "AMD"; MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 2; WarmupTimes = @(45, 0); Arguments = " --algo cn/r" } # Is never profitable
    ...
    [PSCustomObject]@{ Algorithm = "KawPoW";               Type = "AMD"; MinMemGB = $MinerPools[0].KawPoW.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(60, 15); Arguments = " --algo kawpow" } # NBMiner-v42.2 is fastest, but has 2% miner fee
...
#   [PSCustomObject]@{ Algorithm = "RandomX";              Type = "CPU"; MinerSet = 0; WarmupTimes = @(45, 0); Arguments = " --algo rx/0" } # Not profitable at all
    [PSCustomObject]@{ Algorithm = "RandomxArq";           Type = "CPU"; MinerSet = 0; WarmupTimes = @(45, 0); Arguments = " --algo rx/arq" } # SRBMinerMulti-v1.0.2 is fastest, but has 0.85% miner fee
    ...
    [PSCustomObject]@{ Algorithm = "RandomxSfx";           Type = "CPU"; MinerSet = 0; WarmupTimes = @(45, 0); Arguments = " --algo rx/sfx" } # SRBMinerMulti-v1.0.2 is fastest, but has 0.85% miner fee
...
#   [PSCustomObject]@{ Algorithm = "CryptonightR";         Type = "NVIDIA"; MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 0);  Arguments = " --algo cn/r" } # Is never profitable
    ...
    [PSCustomObject]@{ Algorithm = "KawPoW";               Type = "NVIDIA"; MinMemGB = $MinerPools[0].KawPoW.DAGSizeGB; MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(60, 15); Arguments = " --algo kawpow" } # Trex-v0.26.5 is fastest, but has 1% miner fee
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object { $_.Type -eq "CPU" -or $_.MemoryGB -gt $MinMemGB }) { 
                ...
                [PSCustomObject]@{ 
                    ...
                    Path        = $Path
                    Arguments   = ("$($Arguments) $(If ($MinerPools[0].($_.Algorithm).BaseName -eq "NiceHash") { " --nicehash" } )$(If ($MinerPools[0].($_.Algorithm).AvailablePorts[1]) { " --tls" } ) --url=$($MinerPools[0].($_.Algorithm).Host):$($MinerPools[0].($_.Algorithm).AvailablePorts | Where-Object  { $_ -ne $null } | Select-Object -Last 1) --user=$($MinerPools[0].($_.Algorithm).User)$(If ($MinerPools[0].($_.Algorithm).WorkerName) { ".$($MinerPools[0].($_.Algorithm).WorkerName)" }) --pass=$($MinerPools[0].($_.Algorithm).Pass)$(If ($MinerPools[0].($_.Algorithm).WorkerName) { " --rig-id $($MinerPools[0].($_.Algorithm).WorkerName)" }) --keepalive --http-enabled --http-host=127.0.0.1 --http-port=$($MinerAPIPort) --api-worker-id=$($Config.WorkerName) --api-id=$($Miner_Name) --retries=90 --retry-pause=1" -replace "\s+", " ").trim()
