...
$Commands = [PSCustomObject[]]@(
    [PSCustomObject]@{ Algorithm = "Sha3d"; MinMemGB = 1; Command = " --algo keccak --statsavg 5" } # Keccak!!!
)
            ...
            $Commands | Where-Object { -not $Pools.($_.Algorithm).SSL } | ForEach-Object {
                ...
                If ($Miner_Devices = @($SelectedDevices | Where-Object { ($_.OpenCL.GlobalMemSize / 1GB) -ge $MinMemGB })) { 
                    ...
                    [PSCustomObject]@{ 
                        ...
                        Type       = "NVIDIA"
                        Path       = $Path
                        Arguments  = ("$($_.Command) --url stratum+tcp://$($Pools.($_.Algorithm).Host):$($Pools.($_.Algorithm).Port) --user $($Pools.($_.Algorithm).User) --pass $($Pools.($_.Algorithm).Pass) --retry-pause 1 --api-bind $MinerAPIPort --devices $(($Miner_Devices | Sort-Object $DeviceEnumerator | ForEach-Object { '{0:x}' -f $_.$DeviceEnumerator }) -join ',')" -replace "\s+", " ").trim()
