...
$Algorithms = [PSCustomObject[]]@(
    ...
    # [PSCustomObject]@{ Algorithm = "KawPoW";            Fee = 0.01; MinMemGB = ($Pools."KawPoW".DAGSize + $DAGmemReserve) / 1GB;; MinComputeCapability = 5.0; MinerSet = 1; WarmupTimes = @(45, 0); Arguments = " --algo kawpow --intensity 8" } # TTMiner-v5.0.3 is fastest
    [PSCustomObject]@{ Algorithm = "Phi2";              Fee = 0.01; MinMemGB = 2;                                                 MinComputeCapability = 5.0; MinerSet = 0; WarmupTimes = @(30, 0);  Arguments = " --algo phi2 --intensity 8" }
    [PSCustomObject]@{ Algorithm = "MTP";               Fee = 0.02; MinMemGB = 5;                                                 MinComputeCapability = 6.0; MinerSet = 1; WarmupTimes = @(30, 0);  Arguments = " --algo mtp --intensity 8" } # Trex-v0.25.9 is fastest
