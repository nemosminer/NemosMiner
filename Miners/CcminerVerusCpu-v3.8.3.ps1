...
$Algorithms = [PSCustomObject[]]@(
    [PSCustomObject]@{ Algorithm = "VerusHash"; Minerset = 1; WarmupTimes = @(45, 600); ExcludePools = @("NiceHash"); Arguments = " --algo verus" } # SRBMinerMulti-v2.3.7 is fastest, but has 0.85% miner fee
)
...
If ($Algorithms) { 
    ...
    $Algorithms | ForEach-Object { 
        ...
        ForEach ($Pool in ($MinerPools[0][$_.Algorithm] | Where-Object { $_.PoolPorts[0] } | Where-Object BaseName -notin $ExcludePools | Select-Object -Last 1)) { 
            ...
            [PSCustomObject]@{ 
                API         = "CcMiner"
                Arguments   = "$($_.Arguments) --url stratum+tcp://$($Pool.Host):$($Pool.PoolPorts[0]) --user $($Pool.User)$(If ($Pool.WorkerName) { ".$($Pool.WorkerName)" }) --pass $($Pool.Pass) --threads $($AvailableMiner_Devices.CIM.NumberOfLogicalProcessors -1) --statsavg 1 --retry-pause 1 --api-bind $MinerAPIPort"
