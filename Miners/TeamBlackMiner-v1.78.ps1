...
#Temp fix
$SSL = $true # SSL support (miner does not yet fully support SSL)

$Uri = Switch ($Variables.DriverVersion.CUDA) { 
    { $_ -ge "12.0" } { "https://github.com/sp-hash/TeamBlackMiner/releases/download/v1.78/TeamBlackMiner_1_78_cuda_12.7z"; Break }
    Default { "https://github.com/sp-hash/TeamBlackMiner/releases/download/v1.78/TeamBlackMiner_1_78_cuda_11_6.7z" }
}
...
#   [PSCustomObject]@{ Algorithm = "EthashLowMem"; Type = "NVIDIA"; Fee = 0.005; MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB + 0.41; ExcludePool = @();             MinerSet = 1; Tuning = " --tweak 2"; WarmupTimes = @(45, 0); Arguments = " --algo ethash" } # TTMiner-v5.0.3 is fastest
    [PSCustomObject]@{ Algorithm = "KawPow";       Type = "NVIDIA"; Fee = 0.01;  MinMemGB = $MinerPools[0].KawPow.DAGSizeGB + 0.41;       ExcludePool = @("ProHashing"); MinerSet = 1; Tuning = " --tweak 2"; WarmupTimes = @(60, 0); Arguments = " --algo kawpow" }
...
If ($Algorithms = $Algorithms | Where-Object MinerSet -LE $Config.MinerSet | Where-Object { $MinerPools[0].($_.Algorithm).PoolPorts } | Where-Object { $MinerPools[0].($_.Algorithm).PoolPorts[0] -or ($SSL -and $MinerPools[0].($_.Algorithm).PoolPorts[1]) }) { 
