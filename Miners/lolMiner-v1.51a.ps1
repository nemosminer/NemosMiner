...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("CuckarooD29");               Type = "AMD"; Fee = @(0.01);       MinMemGB = 4.0;                             MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(45, 30); Arguments = " --algo C29D" } # TeamRedMiner-v0.10.1 is fastest, keep enabled because TeamRed does not support algo on Navi
    ...
    [PSCustomObject]@{ Algorithm = @("Cuckatoo31");                Type = "AMD"; Fee = @(0.02);       MinMemGB = 4.0;                             MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(60, 60); Arguments = " --algo C31" } # TeamRedMiner-v0.10.1 is fastest
    ...
    # [PSCustomObject]@{ Algorithm = @("EtcHash", "SHA256ton");      Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $Pools."Etchash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); Arguments = " --algo ETCHASH --dualmode TONDUAL" } # No hashrates
    ...
    # [PSCustomObject]@{ Algorithm = @("Ethash", "SHA256ton");       Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $Pools."Ethash".DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); Arguments = " --algo ETHASH --dualmode TONDUAL" } # No hashrates
    ...
    # [PSCustomObject]@{ Algorithm = @("EthashLowMem", "SHA256ton"); Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $Pools."EthashLowMem".DAGSizeGB; MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); Arguments = " --algo ETHASH --dualmode TONDUAL" } # No hashrates
    [PSCustomObject]@{ Algorithm = @("SHA256ton");                 Type = "AMD"; Fee = @(0.01);       MinMemGB = 3.0;                             MemReserveGB = 0;    MinerSet = 2; WarmupTimes = @(45, 30); Arguments = " --algo TON" }
    ...
    [PSCustomObject]@{ Algorithm = @("CuckarooD29");               Type = "NVIDIA"; Fee = @(0.01);       MinMemGB = 4.0;                             MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(45, 30); Arguments = " --algo C29D" } # GMiner-v2.99 is fastest
    ...
    [PSCustomObject]@{ Algorithm = @("SHA256ton");                 Type = "NVIDIA"; Fee = @(0.01);       MinMemGB = 3.0;                             MemReserveGB = 0;    MinerSet = 2; WarmupTimes = @(45, 30); Arguments = " --algo TON" }
    ...
    [PSCustomObject]@{ Algorithm = @("UbqHash", "SHA256ton");      Type = "NVIDIA"; Fee = @(0.01, 0.01); MinMemGB = $Pools."UbqHash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(60, 30); Arguments = " --algo UBQHASH --dualmode TONDUAL" }
)
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB)) { 
                ...
                $_.Arguments += " --user $($Pools.($_.Algorithm[0]).User) --pass $($Pools.($_.Algorithm[0]).pass)$(If ($Pools.($_.Algorithm[0]).BaseName -eq "ProHashing" -and $_.Algorithm[0] -eq "EthashLowMem") { ",l=$((($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum) / 1GB - $_.MemReserveGB)" })"
