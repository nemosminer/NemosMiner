...
$Commands | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    [PSCustomObject]@{
        ...
        HashRates = [PSCustomObject]@{(Get-Algorithm($_)) = $Stats."$($Name)_$(Get-Algorithm($_))_HashRate".Week * .87} # substract 13% (testing shows much higher console speed than Actual Pool speed, reduceing recorded console speed to same as pool)
        API = "Ccminer"
