...
$Algorithms = [PSCustomObject[]]@( 
    ...
    [PSCustomObject]@{ Algorithms = @("HeavyHash", "Autolykos2");    Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Autolykos2.DAGSizeGB;   MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 30); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm heavyhash --algorithm autolykos2" }
    [PSCustomObject]@{ Algorithms = @("Autolykos2", "kHeavyHash");   Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Autolykos2.DAGSizeGB;   MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 30); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm autolykos2 --algorithm kheavyhash" }
    ...
    [PSCustomObject]@{ Algorithms = @("CryptonightTurtle");          Type = "AMD"; Fee = @(0.0085);       MinMemGB = 1;                                     MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 30); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm cryptonight_turtle" } # TeamRedMiner-v0.10.5.1 is fastest
    ...
    [PSCustomObject]@{ Algorithms = @("HeavyHash", "EtcHash");       Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Etchash.DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm heavyhash --algorithm etchash --gpu-auto-tune 2" }
    [PSCustomObject]@{ Algorithms = @("EtcHash", "kHeavyHash");      Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Etchash.DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm etchash --algorithm kheavyhash --gpu-auto-tune 2" }
    ...
    [PSCustomObject]@{ Algorithms = @("Heavyhash", "Ethash");        Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Ethash.DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm heavyhash --algorithm ethash --gpu-auto-tune 2" }
    [PSCustomObject]@{ Algorithms = @("Ethash", "kHeavyhash");       Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].Ethash.DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm ethash --algorithm hkeavyhash --gpu-auto-tune 2" }
    [PSCustomObject]@{ Algorithms = @("EthashLowMem");               Type = "AMD"; Fee = @(0.0065);       MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm ethash" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
    ...
    [PSCustomObject]@{ Algorithms = @("HeavyHash", "EthashLowMem");  Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm heavyhash --algorithm ethash --gpu-auto-tune 2" }
    [PSCustomObject]@{ Algorithms = @("EthashLowMem", "kHeavyHash"); Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm ethash --algorithm heavyhash --gpu-auto-tune 2" }
...
#   [PSCustomObject]@{ Algorithms = @("Keccak");                     Type = "AMD"; Fee = @(0);            MinMemGB = 1;                                     MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 30); ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm keccak" } # ASIC
    ...
    [PSCustomObject]@{ Algorithms = @("Pufferfish2BMB");             Type = "AMD"; Fee = @(0.01);         MinMemGB = 8;                                     MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 0);  ExcludeGPUArchitecture = @(); ExcludePool = @();        Arguments = " --algorithm pufferfish2bmb" }
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Where-Object { $MinerPools[0].($_.Algorithm).BaseName -notin $_.ExcludePool } | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object { $_.Type -eq "CPU" -or ($_.MemoryGB -gt $MinMemGB) } | Where-Object { $_.Architecture -notin $ExcludeGPUArchitecture }) { 
                ...
                $Arguments += " --pool $($MinerPools[0].($_.Algorithms[0]).Host):$($MinerPools[0].($_.Algorithms[0]).PoolPorts | Select-Object -Last 1) --wallet $($MinerPools[0].($_.Algorithms[0]).User)"
                If ($MinerPools[0].($_.Algorithms[0]).WorkerName) { " --worker $($MinerPools[0].($_.Algorithms[0]).WorkerName)" }
                $Arguments += " --password $($MinerPools[0].($_.Algorithms[0]).Pass)$(If ($MinerPools[0].($_.Algorithms[0]).BaseName -eq "ProHashing" -and $_.Algorithm -eq "EthashLowMem") { ",l=$((($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum) / 1GB - $_.MemReserveGB)" })"
                If ($MinerPools[0].($_.Algorithms[0]).PoolPorts[1]) { $Arguments += " --tls true" }
                If ($MinerPools[0].($_.Algorithms[0]).DAGsizeGB -ne $null -and $MinerPools[0].($_.Algorithms[0]).BaseName -in @("MiningPoolHub", "NiceHash", "ProHashing")) { $Arguments += " --nicehash true" }
                If ($MinerPools[0].($_.Algorithms[0]).DAGsizeGB -ne $null -and $MinerPools[0].($_.Algorithms[0]).BaseName -eq "ProHashing") { $Arguments += " --esm 1" }

                If ($_.Algorithms[1]) { 
                    $Arguments += " --pool $($MinerPools[0].($_.Algorithms[1]).Host):$($MinerPools[0].($_.Algorithms[1]).PoolPorts | Select-Object -Last 1) --wallet $($MinerPools[0].($_.Algorithms[1]).User)"
                    If ($MinerPools[0].($_.Algorithms[1]).WorkerName) { $Arguments += " --worker $($MinerPools[0].($_.Algorithms[1]).WorkerName)" }
                    $Arguments += " --password $($MinerPools[0].($_.Algorithms[1]).Pass)"
                    If ($MinerPools[0].($_.Algorithms[1]).PoolPorts[1]) { $Arguments += " --tls true" }
                    If ($MinerPools[0].($_.Algorithms[1]).DAGsizeGB -ne $null -and $MinerPools[0].($_.Algorithms[1]).BaseName -in @("MiningPoolHub", "NiceHash", "ProHashing")) { $Arguments += " --nicehash true" }
                    If ($MinerPools[0].($_.Algorithms[1]).DAGsizeGB -ne $null -and $MinerPools[0].($_.Algorithms[1]).BaseName -eq "ProHashing") { $Arguments += " --esm 1" }
                }
