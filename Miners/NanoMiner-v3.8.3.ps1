...
$Uri = "https://github.com/nanopool/nanominer/releases/download/v3.8.3/nanominer-windows-3.8.3.zip"
...
#   [PSCustomObject]@{ Algorithms = @("EthashLowMem", "kHeavyHash"); Type = "AMD"; Fee = 0.01;  MinMemGiB = 1.24; Minerset = 2; Tuning = " -coreClocks +20 -memClocks +100 -memTweak 2"; WarmupTimes = @(45, 60); ExcludePools = @(@(), @());                                           ExcludeGPUArchitecture = @(); Arguments = @(" -algo Ethash"," -algo Kaspa") } # PhoenixMiner-v6.2c is fastest
    ...
    [PSCustomObject]@{ Algorithms = @("KawPow");                     Type = "AMD"; Fee = 0.02;  MinMemGiB = 1.08; Minerset = 2; Tuning = " -coreClocks +20 -memClocks +100 -memTweak 2"; WarmupTimes = @(45, 0);  ExcludePools = @(@("HashCryptos", "MiningDutch", "ProHashing"), @()); ExcludeGPUArchitecture = @(); Arguments = @(" -algo KawPow") } # TeamRedMiner-v0.10.13 is fastest
