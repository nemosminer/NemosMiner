...
#   [PSCustomObject]@{ Algorithm = @("DynamoCoin");          Type = "AMD"; Fee = 0.01;   MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 15); Arguments = " --algorithm dynamo" } # Algorithm 'dynamo' supports only 'pool' mode (yiimp stratum compatibility removed)
    ...
    [PSCustomObject]@{ Algorithm = @("FiroPoW");             Type = "AMD"; Fee = 0.0085; MinMemGB = $Pools."FiroPoW".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(90, 75); Arguments = " --algorithm firopow --gpu-boost 50 --gpu-auto-tune 2" }
...
#   [PSCustomObject]@{ Algorithm = @("Blake2b");              Type = "CPU"; Fee = 0;      MinerSet = 0; WarmupTimes = @(90, 15);   Arguments = " --algorithm blake2b" } # Algo broken
    ...
    [PSCustomObject]@{ Algorithm = @("CryptonightHeavyxXhv"); Type = "CPU"; Fee = 0.0085; MinerSet = 0; WarmupTimes = @(30, 15);   Arguments = " --algorithm cryptonight_xhv" }
...
If ($Algorithms = $Algorithms | Where-Object MinerSet -LE $Config.MinerSet | Where-Object { $Pools.($_.Algorithm[0]).Host }) { 
