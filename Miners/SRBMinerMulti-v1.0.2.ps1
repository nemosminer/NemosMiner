...
$Algorithms = [PSCustomObject[]]@( 
     ...
     [PSCustomObject]@{ Algorithm = @("Argon2d16000");              Type = "AMD"; Fee = @(0.0085);       MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(60, 45); ExcludePool = @()       ; Arguments = " --algorithm argon2d_16000" }
     [PSCustomObject]@{ Algorithm = @("Argon2dDyn");                Type = "AMD"; Fee = @(0.0085);       MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(60, 45); ExcludePool = @()       ; Arguments = " --algorithm argon2d_dynamic" }
...
#    [PSCustomObject]@{ Algorithm = @("Autolykos2");                Type = "AMD"; Fee = @(0.015);        MinMemGB = $Pools."Autolykos2".DAGSizeGB;   MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm autolykos2" } # Hashrate far too high
#    [PSCustomObject]@{ Algorithm = @("Autolykos2", "HeavyHash");   Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $Pools."Autolykos2".DAGSizeGB;   MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm autolykos2 --algorithm heavyhash --gpu-dual-max-loss 5 --gpu-auto-tune 2" }
     ...
     [PSCustomObject]@{ Algorithm = @("Blake2s");                   Type = "AMD"; Fee = @(0);            MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm blake2s" }
     ...
     [PSCustomObject]@{ Algorithm = @("CryptonightUpx");            Type = "AMD"; Fee = @(0.0085);       MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 30); ExcludePool = @()       ; Arguments = " --algorithm cryptonight_upx" } # TeamRedMiner-v0.10.2 is fastest
...
#    [PSCustomObject]@{ Algorithm = @("DynamoCoin");                Type = "AMD"; Fee = @(0.01);         MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 30); ExcludePool = @()       ; Arguments = " --algorithm dynamo" } # Algorithm 'dynamo' supports only 'pool' mode (yiimp stratum compatibility removed)
     [PSCustomObject]@{ Algorithm = @("EtcHash");                   Type = "AMD"; Fee = @(0.0065);       MinMemGB = $Pools."EtcHash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm etchash" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
#    [PSCustomObject]@{ Algorithm = @("EtcHash", "HeavyHash");      Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $Pools."EtcHash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm etchash --algorithm heavyhash --gpu-dual-max-loss 5 --gpu-auto-tune 2" }
     [PSCustomObject]@{ Algorithm = @("Ethash");                    Type = "AMD"; Fee = @(0.0065);       MinMemGB = $Pools."Ethash".DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm ethash" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
#    [PSCustomObject]@{ Algorithm = @("Ethash", "Heavyhash");       Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $Pools."Ethash".DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm ethash --algorithm heavyhash --gpu-dual-max-loss 5 --gpu-auto-tune 2" }
     [PSCustomObject]@{ Algorithm = @("EthashLowMem");              Type = "AMD"; Fee = @(0.0065);       MinMemGB = $Pools."EthashLowMem".DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm ethash" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
#    [PSCustomObject]@{ Algorithm = @("EthashLowMem", "HeavyHash"); Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $Pools."EthashLowMem".DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm ethash --algorithm heavyhash --gpu-dual-max-loss 5 --gpu-auto-tune 2" }
     [PSCustomObject]@{ Algorithm = @("FiroPoW");                   Type = "AMD"; Fee = @(0.0085);       MinMemGB = $Pools."FiroPoW".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm firopow" }
     ...
     [PSCustomObject]@{ Algorithm = @("HeavyHash");                 Type = "AMD"; Fee = @(0.01);         MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm heavyhash" }
     ...
     [PSCustomObject]@{ Algorithm = @("KawPoW");                    Type = "AMD"; Fee = @(0.085);        MinMemGB = $Pools."KawPoW".DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm kawpow" }
 #   [PSCustomObject]@{ Algorithm = @("Keccak");                    Type = "AMD"; Fee = @(0);            MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 30); ExcludePool = @()       ; Arguments = " --algorithm keccak" } # ASIC
     ...
     [PSCustomObject]@{ Algorithm = @("ProgPoWEpic");               Type = "AMD"; Fee = @(0.0065);       MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm progpow_epic" }
     [PSCustomObject]@{ Algorithm = @("ProgPoWSero");               Type = "AMD"; Fee = @(0.0065);       MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm progpow_sero" }
     [PSCustomObject]@{ Algorithm = @("ProgPoWVeil");               Type = "AMD"; Fee = @(0.0065);       MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm progpow_veil" }
     ...
     [PSCustomObject]@{ Algorithm = @("ProgPoWZano");               Type = "AMD"; Fee = @(0.0065);       MinMemGB = 2;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm progpow_zano" }
     [PSCustomObject]@{ Algorithm = @("SHA3d");                     Type = "CPU"; Fee = @(0.0085);       MinMemGB = 1;                               MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 30); ExcludePool = @()       ; Arguments = " --algorithm sha3d" }
    ...
    [PSCustomObject]@{ Algorithm = @("Argon2dDyn");           Type = "CPU"; Fee = 0.0085; MinerSet = 0; WarmupTimes = @(60, 15);  ExcludePool = @()       ; Arguments = " --algorithm argon2d_dynamic" }
...
#   [PSCustomObject]@{ Algorithm = @("Autolykos2");           Type = "CPU"; Fee = 0.015;  MinerSet = 0; WarmupTimes = @(120, 15); ExcludePool = @()       ; Arguments = " --algorithm autolykos2 --gpu-autolykos2-preload 3" } # Not profitable with CPU
...
#   [PSCustomObject]@{ Algorithm = @("FiroPoW");              Type = "CPU"; Fee = 0.0085; MinerSet = 0; WarmupTimes = @(30, 0);   ExcludePool = @()       ; Arguments = " --algorithm firopow" } # Not profitable with CPU
    ...
    [PSCustomObject]@{ Algorithm = @("HeavyHash");            Type = "CPU"; Fee = 0.01;   MinerSet = 0; WarmupTimes = @(30, 0);   ExcludePool = @()       ; Arguments = " --algorithm heavyhash" }
...
#   [PSCustomObject]@{ Algorithm = @("KawPoW");               Type = "CPU"; Fee = 0.085;  MinerSet = 0; WarmupTimes = @(60, 0);   ExcludePool = @()       ; Arguments = " --algorithm kawpow" } # Not profitable with CPU
    [PSCustomObject]@{ Algorithm = @("Kaspa");                Type = "CPU"; Fee = 0;      MinerSet = 0; WarmupTimes = @(30, 0);   ExcludePool = @()       ; Arguments = " --algorithm kaspa" }
#   [PSCustomObject]@{ Algorithm = @("Keccak");               Type = "CPU"; Fee = 0;      MinerSet = 0; WarmupTimes = @(30, 0);   ExcludePool = @()       ; Arguments = " --algorithm keccak" } # ASIC
    ...
    [PSCustomObject]@{ Algorithm = @("ProgPoWZano");          Type = "CPU"; Fee = 0.0085; MinerSet = 0; WarmupTimes = @(30, 0);   ExcludePool = @()       ; Arguments = " --algorithm progpow_zano" }
...
If ($Algorithms = $Algorithms | Where-Object MinerSet -LE $Config.MinerSet | Where-Object { $Pools.($_.Algorithm[0]).Host -and (-not $_.Algorithm[1] -or $PoolsSecondaryAlgorithm.($_.Algorithm[1]).Host) }) { 
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Where-Object { $Pools.($_.Algorithm).BaseName -notin $_.ExcludePool } | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object { $_.Type -eq "CPU" -or ($_.MemoryGB -gt $MinMemGB) }) { 
                ...
                $_.Arguments += " --pool $($Pools.($_.Algorithm[0]).Host):$($Pools.($_.Algorithm[0]).Port) --wallet $($Pools.($_.Algorithm[0]).User)"
                If ($Pools.($_.Algorithm[0]).WorkerName) { " --worker $($Pools.($_.Algorithm[0]).WorkerName)" }
                $_.Arguments += " --password $($Pools.($_.Algorithm[0]).Pass)$(If ($Pools.($_.Algorithm[0]).BaseName -eq "ProHashing" -and $_.Algorithm -eq "EthashLowMem") { ",l=$((($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum) / 1GB - $_.MemReserveGB)" })"
                If ($Pools.($_.Algorithm[0]).SSL) { $_.Arguments += " --ssl true" }
                If ($Pools.($_.Algorithm[0]).DAGsizeGB -ne $null -and $Pools.($_.Algorithm[0]).BaseName -in @("MiningPoolHub", "NiceHash", "ProHashing")) { $_.Arguments += " --nicehash true" }
                If ($Pools.($_.Algorithm[0]).DAGsizeGB -ne $null -and $Pools.($_.Algorithm[0]).BaseName -eq "ProHashing") { $_.Arguments += " --esm 1" }

                If ($_.Algorithm[1]) { 
                    $_.Arguments += " --pool $($Pools.($_.Algorithm[1]).Host):$($Pools.($_.Algorithm[1]).Port) --wallet $($Pools.($_.Algorithm[1]).User)"
                    If ($Pools.($_.Algorithm[1]).WorkerName) { " --worker $($Pools.($_.Algorithm[1]).WorkerName)" }
                    $_.Arguments += " --password $($Pools.($_.Algorithm[1]).Pass)"
                    If ($Pools.($_.Algorithm[1]).SSL) { $_.Arguments += " --ssl true" }
                    If ($Pools.($_.Algorithm[1]).DAGsizeGB -ne $null -and $Pools.($_.Algorithm[1]).BaseName -in @("MiningPoolHub", "NiceHash", "ProHashing")) { $_.Arguments += " --nicehash true" }
                    If ($Pools.($_.Algorithm[1]).DAGsizeGB -ne $null -and $Pools.($_.Algorithm[1]).BaseName -eq "ProHashing") { $_.Arguments += " --esm 1" }
                }
