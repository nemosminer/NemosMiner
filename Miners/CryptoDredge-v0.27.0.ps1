    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object { $MinerPools[0].($_.Algorithm).BaseName -notin $_.ExcludePool } | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB) | Where-Object { [Double]$_.OpenCL.ComputeCapability -ge $MinComputeCapability }) { 
                ...
                $_.Arguments += " --url stratum+$(If ($MinerPools[0].($_.Algorithm).SSL) { "ssl" } Else { "tcp" } )://$($MinerPools[0].($_.Algorithm).Host):$($MinerPools[0].($_.Algorithm).Port) --user $($MinerPools[0].($_.Algorithm).User)"
