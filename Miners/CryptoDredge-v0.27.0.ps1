...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "Argon2d500";        Fee = 0.01; MinMemGB = 2;                                            ExcludePool = @();                            MinerSet = 0; WarmupTimes = @(60, 0);  Arguments = " --algo argon2d-dyn --intensity 6" }
    ...
    [PSCustomObject]@{ Algorithm = "Ethash";            Fee = 0.01; MinMemGB = $MinerPools[0].Ethash.DAGSizeGB + 0.42;       ExcludePool = @("MiningPoolHub", "NiceHash"); MinerSet = 1; WarmupTimes = @(45, 0);  Arguments = " --algo ethash" }
#   [PSCustomObject]@{ Algorithm = "EthashLowMem";      Fee = 0.01; MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB + 0.42; ExcludePool = @("MiningPoolHub", "NiceHash"); MinerSet = 1; WarmupTimes = @(45, 0);  Arguments = " --algo ethash" }
    [PSCustomObject]@{ Algorithm = "FiroPow";           Fee = 0.01; MinMemGB = $MinerPools[0].FiroPow.DAGSizeGB + 0.50;      ExcludePool = @();                            MinerSet = 0; WarmupTimes = @(45, 0);  Arguments = " --algo firopow" }
    [PSCustomObject]@{ Algorithm = "KawPow";            Fee = 0.01; MinMemGB = $MinerPools[0].KawPow.DAGSizeGB + 0.42;       ExcludePool = @("MiningPoolHub");             MinerSet = 1; WarmupTimes = @(45, 0);  Arguments = " --algo kawpow --intensity 8" } # TTMiner-v5.0.3 is fastest
)
