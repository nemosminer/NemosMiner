    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -eq $_.Type | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGiB -GE $_.MinMemGiB) { 
                ...
                $Threads = [Math]::Ceiling(($AvailableMiner_Devices | ForEach-Object { ($_.MemoryGiB - 1) / 4 } | Measure-Object -Minimum).Minimum)

                # Reserve 250KB for AMD driver, for NVIDIA
                $GPUmemory = ($AvailableMiner_Devices | ForEach-Object { $_.MemoryGiB } | Measure-Object -Minimum).Minimum
                ...
                [PSCustomObject]@{ 
                    ...
                    API         = "CcMiner"
                    Arguments   = ("$($Arguments) --url stratum+tcp://$($AllMinerPools.($_.Algorithm).Host):$($AllMinerPools.($_.Algorithm).PoolPorts[0]) --user $($AllMinerPools.($_.Algorithm).User) --pass $($AllMinerPools.($_.Algorithm).Pass)$(If ($AllMinerPools.($_.Algorithm).WorkerName) { ".$($AllMinerPools.($_.Algorithm).WorkerName)" }) --gpu-batchsize $BatchSize --threads $Threads --retry-pause 1 --api-bind 127.0.0.1:$($MinerAPIPort) --gpu-id $(($AvailableMiner_Devices.($DeviceEnumerator.($AvailableMiner_Devices.Type | Select-Object -Unique)) | Sort-Object -Unique | ForEach-Object { '{0:x}' -f ($_ + 1)}) -join ',')" -replace "\s+", " ").trim()
