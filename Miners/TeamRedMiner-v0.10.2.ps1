...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "CryptonightCcx";       Fee = @(0.025); MinMemGB = 2.0;                                   MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(60, 15);   ExcludeGPUArchitecture = @("RDNA"); Arguments = " --algo=cn_conceal --auto_tune=QUICK --auto_tune_runs=2 --allow_large_alloc --no_lean" } # SRBMinerMulti-v1.0.2 is fastest
...
#   [PSCustomObject]@{ Algorithm = "CryptonightR";         Fee = @(0.025); MinMemGB = 2.0;                                   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(60, 15);   ExcludeGPUArchitecture = @("RDNA"); Arguments = " --algo=cnr --auto_tune=QUICK --auto_tune_runs=2 --allow_large_alloc --no_lean" } # Not profitable at all
    ...
    [PSCustomObject]@{ Algorithm = "Cuckatoo31";           Fee = @(0.025); MinMemGB = 3.0;                                   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(60, 15);   ExcludeGPUArchitecture = @("RDNA"); Arguments = " --algo=cuckatoo31_grin" } # lolMiner-v1.54 is fastest
    ...
    [PSCustomObject]@{ Algorithm = "FiroPoW";              Fee = @(0.02);  MinMemGB = $MinerPools[0].FiroPoW.DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 75);   ExcludeGPUArchitecture = @();       Arguments = " --algo=firopow" } # Wildrig-v0.32.0 is fastest on Polaris
    [PSCustomObject]@{ Algorithm = "KawPoW";               Fee = @(0.02);  MinMemGB = $MinerPools[0].KawPoW.DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(60, 75);   ExcludeGPUArchitecture = @();       Arguments = " --algo=kawpow" } # Wildrig-v0.32.0 is fastest on Polaris
    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB) | Where-Object { $_.Architecture -notin $ExcludeGPUArchitecture }) { 
                ...
                $_.Arguments += " --url=$(If ($MinerPools[0].($_.Algorithm).AvailablePorts[1]) { "stratum+ssl" } Else { "stratum+tcp" })://$($MinerPools[0].($_.Algorithm).Host):$($MinerPools[0].($_.Algorithm).AvailablePorts | Select-Object -Last 1)"
