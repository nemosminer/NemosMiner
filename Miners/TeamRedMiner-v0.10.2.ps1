...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("Cuckatoo31");           Fee = @(0.025); MinMemGB = 3.0;                             MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(60, 15);   Arguments = " --algo=cuckatoo31_grin" } # lolMiner-v1.52 is fastest
    ...
    [PSCustomObject]@{ Algorithm = @("FiroPoW");              Fee = @(0.02);  MinMemGB = $Pools."FiroPoW".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 75);   Arguments = " --algo=firopow" } # Wildrig-v0.31.3 is fastest on Polaris
    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices) { 
                ...
                $_.Arguments += " --user=$($Pools.($_.Algorithm[0]).User)$(If ($Pools.($_.Algorithm[0]).Pass) { " --pass=$($Pools.($_.Algorithm[0]).Pass)" })$(If ($Pools.($_.Algorithm[0]).BaseName -eq "ProHashing" -and $_.Algorithm -eq "EthashLowMem") { ",l=$((($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum) / 1GB - $_.MemReserveGB)" })"

                If ($_.Algorithm[1] -eq "SHA256ton") { 
                    If ($Pools.($_.Algorithm[1]).BaseName -eq "TonWhales") { 
                        $_.Arguments += " --ton_start --url=stratum+tcp://tcp.$($Pools.($_.Algorithm[1]).Host):$($Pools.($_.Algorithm[1]).Port) --user=$($Pools.($_.Algorithm[1]).User)$(If ($Pools.($_.Algorithm[0]).Pass) { " --pass=$($Pools.($_.Algorithm[0]).Pass)" }) --api2_listen=127.0.0.1:$($MinerAPIPort + 1) --ton_end"
                    }
                    Else { 
                        $_.Arguments += " --ton_start --url=stratum+tcp://$($Pools.($_.Algorithm[1]).Host):$($Pools.($_.Algorithm[1]).Port) --user=$($Pools.($_.Algorithm[1]).User)$(If ($Pools.($_.Algorithm[1]).Pass) { " --pass=$($Pools.($_.Algorithm[1]).Pass)" }) --api2_listen=127.0.0.1:$($MinerAPIPort + 1) --ton_end"
                    }
                }
