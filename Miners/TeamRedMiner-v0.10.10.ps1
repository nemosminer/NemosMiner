...
$Uri = "https://github.com/todxx/teamredminer/releases/download/v0.10.10/teamredminer-v0.10.10-win.zip"
...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithms = @("Autolykos2", "Blake3IronFish");   Fee = @(0.025); MinMemGiB = $MinerPools[0].Autolykos2.DAGSizeGiB + 0.77;   Minerset = 2; WarmupTimes = @(60, 15);  ExcludePools = @(@(), @());                ExcludeGPUArchitecture = @("CGN1", "CGN2");                            Arguments = " --algo=autolykos2" } # Pools with support at this time are Herominers, Flexpool and Kryptex
#   [PSCustomObject]@{ Algorithms = @("Blake3IronFish");                 Fee = @(0.025); MinMemGiB = 2.0;                                           Minerset = 2; WarmupTimes = @(60, 15);  ExcludePools = @(@(), @());                ExcludeGPUArchitecture = @("CGN1", "CGN2", "RDNA1", "RDNA2", "RDNA3"); Arguments = " --algo=ironfish" } # Pools with support at this time are Herominers, Flexpool and Kryptex
...
#   [PSCustomObject]@{ Algorithms = @("Ethash", "Blake3IronFish");       Fee = @(0.01);  MinMemGiB = $MinerPools[0].Ethash.DAGSizeGiB + 0.77;       Minerset = 2; WarmupTimes = @(60, 65);  ExcludePools = @(@(), @());                ExcludeGPUArchitecture = @("CGN1", "CGN2");                            Arguments = " --algo=ethash" } # Pools with support at this time are Herominers, Flexpool and Kryptex
...
#   [PSCustomObject]@{ Algorithms = @("EthashLowMem", "Blake3IronFish"); Fee = @(0.01);  MinMemGiB = $MinerPools[0].EthashLowMem.DAGSizeGiB + 0.77; Minerset = 2; WarmupTimes = @(45, 65);  ExcludePools = @(@(), @());                ExcludeGPUArchitecture = @("CGN1", "CGN2");                            Arguments = " --algo=ethash" } # Pools with support at this time are Herominers, Flexpool and Kryptex
    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGiB -ge $_.MinMemGiB | Where-Object Architecture -notin $_.ExcludeGPUArchitecture) { 
                ...
                If ($Algorithm1 -eq "Blake3IronFish") { 
                    $Arguments += " --iron" 
                    $Arguments += " --url=$(If ($MinerPools[1].$Algorithm1.PoolPorts[1]) { "stratum+ssl" } Else { "stratum+tcp" })://$($MinerPools[1].$Algorithm1.Host):$($MinerPools[1].$Algorithm1.PoolPorts | Select-Object -Last 1)"
                    $Arguments += " --user=$($MinerPools[1].$Algorithm1.User)$(If ($MinerPools[1].$Algorithm1.WorkerName) { ".$($MinerPools[1].$Algorithm1.WorkerName)" })"
                    $Arguments += " --pass=$($MinerPools[1].$Algorithm1.Pass)"
