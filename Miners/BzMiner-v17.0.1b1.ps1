...
$Algorithms = @(
    ...
    [PSCustomObject]@{ Algorithms = @("kHeavyHash");         Type = "AMD"; Fee = @(0.01);       MinMemGiB = 2;    Minerset = 2; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 0);  ExcludeGPUArchitecture = @();       ExcludeGPUModel = ""; ExcludePools = @(@("ProHashing"), @()); Arguments = @(" -a kaspa") }
    ...
    [PSCustomObject]@{ Algorithms = @("KawPow");                   Type = "NVIDIA"; Fee = @(0.01, 0.01);   MinMemGiB = 1.08; MinerSet = 1; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(75, 0);  ExcludeGPUArchitecture = @(); ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@(), @());             Arguments = @(" -a rvn") }
    [PSCustomObject]@{ Algorithms = @("kHeavyHash");               Type = "NVIDIA"; Fee = @(0.01, 0.01);   MinMemGiB = 2;    Minerset = 2; Tuning = " --oc_mem_tweak 2"; WarmupTimes = @(45, 0);  ExcludeGPUArchitecture = @(); ExcludeGPUModel = "^MX[1|2]\d+"; ExcludePools = @(@("ProHashing"), @()); Arguments = @(" -a kaspa") }
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = If ($_.ExcludeGPUModel) { $Miner_Devices | Where-Object Model -notmatch $_.ExcludeGPUModel } Else { $Miner_Devices }) { 
                ...
                ForEach ($Pool0 in ($MinerPools[0][$_.Algorithms[0]] | Where-Object Name -notin $ExcludePools[0] | Where-Object { $_."Ethash".Epoch -eq $null -or $_."Ethash".Epoch -gt 0 } | Where-Object { $Config.SSL -ne "Always" -or $_.SSLSelfSignedCertificate -ne $true })) { 
