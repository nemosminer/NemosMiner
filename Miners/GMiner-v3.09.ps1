...
$Uri = "https://github.com/develsoftware/GMinerRelease/releases/download/3.09/gminer_3_09_windows64.zip"
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        ($Algorithms | Where-Object Type -EQ $_.Type).PSObject.Copy() | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB)) { 
                ...
                $_.Arguments += " --server $($MinerPools[0].($_.Algorithms[0]).Host):$($MinerPools[0].($_.Algorithms[0]).PoolPorts | Select-Object -Last 1)"
                If ($MinerPools[0].($_.Algorithms[0]).DAGsizeGB -ne $null -and $MinerPools[0].($_.Algorithms[0]).BaseName -in @("MiningPoolHub", "NiceHash")) { $_.Arguments += " --proto stratum" }
                If ($MinerPools[0].($_.Algorithms[0]).PoolPorts[1]) { $_.Arguments += " --ssl 1" }
                ...
                If ($_.Algorithms[1]) { 
                    $_.Arguments += " --dserver $($MinerPools[0].($_.Algorithms[1]).Host):$($MinerPools[0].($_.Algorithms[1]).PoolPorts | Select-Object -Last 1)"
                    If ($MinerPools[0].($_.Algorithms[1]).PoolPorts[1]) { $_.Arguments += " --dssl 1" }
                    $_.Arguments += " --duser $($MinerPools[0].($_.Algorithms[1]).User)"
                    $_.Arguments += " --dpass $($MinerPools[0].($_.Algorithms[1]).Pass)"
                    If ($MinerPools[0].($_.Algorithms[1]).WorkerName) { $_.Arguments += " --dworker $($MinerPools[0].($_.Algorithms[1]).WorkerName)" }
