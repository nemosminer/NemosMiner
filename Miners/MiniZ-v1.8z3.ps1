...
$Algorithms = [PSCustomObject[]]@(
    [PSCustomObject]@{ Algorithm = "BeamV3";           Type = "AMD"; Fee = @(0.02);   MinMemGB = 4.0;                                   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 15); ExcludeGPUArchitecture = @("CGN4", "RDNA"); Arguments = " --amd --par=beam3 --pers=Beam-PoW" } # NBMiner-v42.3 is fastest
    ...
    [PSCustomObject]@{ Algorithm = "Equihash1445";     Type = "AMD"; Fee = @(0.02);   MinMemGB = 2.0;                                   MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 15); ExcludeGPUArchitecture = @("RDNA");         Arguments = " --amd --par=144,5 $(Get-EquihashCoinPers -Command "--pers " -Currency $MinerPools[0].Equihash1445.Currency -DefaultCommand "--smart-pers")" }
    ...
    [PSCustomObject]@{ Algorithm = "Equihash965";      Type = "AMD"; Fee = @(0.02);   MinMemGB = 2.0;                                   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 15); ExcludeGPUArchitecture = @("RDNA");         Arguments = " --amd --par=96,5 --smart-pers" }
    ...
    [PSCustomObject]@{ Algorithm = "ProgPoWZ";         Type = "AMD"; Fee = @(0.01);   MinMemGB = $MinerPools[0].ProgPoWZ.DAGSizeGB;     MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(30, 15); ExcludeGPUArchitecture = @("CGN4", "RDNA"); Arguments = " --amd --pers=zano" }

    [PSCustomObject]@{ Algorithm = "BeamV3";           Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 4.0;                                   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 15); Arguments = " --nvidia --par=beam3 --pers=Beam-PoW --ocX" } # NBMiner-v42.3 is fastest
    ...
    [PSCustomObject]@{ Algorithm = "Equihash1445";     Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 2.0;                                   MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 15); Arguments = " --nvidia --par=144,5 $(Get-EquihashCoinPers -Command "--pers " -Currency $MinerPools[0].Equihash1445.Currency -DefaultCommand "--smart-pers") --ocX" }
    ...
    [PSCustomObject]@{ Algorithm = "Equihash1927";     Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 2.3;                                   MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(30, 15); Arguments = " --nvidia --par=192,7 $(Get-EquihashCoinPers -Command "--pers " -Currency $MinerPools[0].Equihash1927.Currency -DefaultCommand "--smart-pers") --ocX" }
    ...
    [PSCustomObject]@{ Algorithm = "ProgPoWSero";      Type = "NVIDIA"; Fee = @(0.01);   MinMemGB = $MinerPools[0].ProgPoWSero.DAGSizeGB;  MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(30, 15); Arguments = " --nvidia --pers=sero --ocX" }
    ...
    [PSCustomObject]@{ Algorithm = "ProgPoWZ";         Type = "NVIDIA"; Fee = @(0.01);   MinMemGB = $MinerPools[0].ProgPoWZ.DAGSizeGB;     MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(30, 15); Arguments = " --nvidia --pers=zano --ocX" }
)
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB) | Where-Object { $_.Architecture -notin $ExcludeGPUArchitecture }) { 
                ...
                $_.Arguments += " --url $(If ($MinerPools[0].($_.Algorithm).PoolPorts[1]) { "ssl://" } )$($MinerPools[0].($_.Algorithm).User)$(If ($MinerPools[0].($_.Algorithm).WorkerName) { ".$($MinerPools[0].($_.Algorithm).WorkerName)" })@$($MinerPools[0].($_.Algorithm).Host):$($MinerPools[0].($_.Algorithm).PoolPorts | Select-Object -Last 1)"
                ...
                [PSCustomObject]@{ 
                    ...
                    Path            = $Path
                    Arguments       = ("$($_.Arguments) --jobtimeout=900 --retries=99 --retrydelay=1 --stat-int 10 --latency --all-shares --extra --tempunits C --show-pers --fee-time=60 --telemetry $MinerAPIPort -cd $(($AvailableMiner_Devices.$DeviceEnumerator | Sort-Object -Unique | ForEach-Object { '{0:d2}' -f $_ }) -join ' ')" -replace "\s+", " ").trim()
