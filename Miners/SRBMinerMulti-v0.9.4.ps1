...
$Algorithms = [PSCustomObject[]]@( 
    ...
    [PSCustomObject]@{ Algorithm = "Autolykos2";        Fee = @(0.02; ); MinMemGB = $MinerPools[0].Autolykos2.DAGSizeGB + 0.42;   MinerSet = 0; WarmupTimes = @(45, 30); Arguments = " --algorithm autolykos2" }
...
#   [PSCustomObject]@{ Algorithm = "DynamoCoin";        Fee = @(0.01); ; MinMemGB = 1;                                            MinerSet = 0; WarmupTimes = @(30, 30); Arguments = " --algorithm dynamo" } # Algorithm 'dynamo' supports only 'pool' mode (yiimp stratum compatibility removed)
    [PSCustomObject]@{ Algorithm = "EtcHash";           Fee = @(0.0065); MinMemGB = $MinerPools[0].Etchash.DAGSizeGB + 0.42;      MinerSet = 1; WarmupTimes = @(90, 75); Arguments = " --algorithm etchash --gpu-boost 50" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
    [PSCustomObject]@{ Algorithm = "Ethash";            Fee = @(0.0065); MinMemGB = $MinerPools[0].Ethash.DAGSizeGB + 0.42;       MinerSet = 1; WarmupTimes = @(90, 75); Arguments = " --algorithm ethash --gpu-boost 50" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
#   [PSCustomObject]@{ Algorithm = "EthashLowMem";      Fee = @(0.0065); MinMemGB = $MinerPools[0].EthashLowMem.DAGSizeGB + 0.42; MinerSet = 1; WarmupTimes = @(90, 75); Arguments = " --algorithm ethash --gpu-boost 50" } # PhoenixMiner-v6.2c may be faster, but I see lower speed at the pool
    [PSCustomObject]@{ Algorithm = "FiroPow";           Fee = @(0.0085); MinMemGB = $MinerPools[0].FiroPow.DAGSizeGB + 0.42;      MinerSet = 0; WarmupTimes = @(60, 75); Arguments = " --algorithm firopow --gpu-boost 50" }
    ...
    [PSCustomObject]@{ Algorithm = "KawPow";            Fee = @(0.0085); MinMemGB = $MinerPools[0].KawPow.DAGSizeGB + 0.42;       MinerSet = 0; WarmupTimes = @(90, 75); Arguments = " --algorithm kawpow --gpu-boost 50" }
#   [PSCustomObject]@{ Algorithm = "Keccak";            Fee = @(0);    ; MinMemGB = 1;                                            MinerSet = 0; WarmupTimes = @(30, 30); Arguments = " --algorithm keccak" } # ASIC
    ...
    [PSCustomObject]@{ Algorithm = "ProgPowSero";       Fee = @(0.0065); MinMemGB = $MinerPools[0].ProgPowSero.DAGSizeGB + 0.42;  MinerSet = 0; WarmupTimes = @(45, 30); Arguments = " --algorithm progpow_sero" }
    ...
    [PSCustomObject]@{ Algorithm = "ProgPowanoZ";       Fee = @(0.0065); MinMemGB = $MinerPools[0].ProgPowZ.DAGSizeGB + 0.42;     MinerSet = 0; WarmupTimes = @(45, 30); Arguments = " --algorithm progpow_zano" }
    ...
    [PSCustomObject]@{ Algorithm = "VertHash";          Fee = @(0.0125); MinMemGB = 1;                                            MinerSet = 0; WarmupTimes = @(30, 30); Arguments = " --algorithm verthash --verthash-dat-path ..\..\Cache\VertHash.dat" }
