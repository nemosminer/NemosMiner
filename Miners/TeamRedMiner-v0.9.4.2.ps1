...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("EtcHash");              Fee = @(0.01);  MinMemGB = ($Pools."EtcHash".DAGSize + 0.95GB) / 1GB;      MinerSet = 1; WarmupTimes = @(45, 75); Arguments = " --algo=etchash" } # PhoenixMiner-v6.0c is fastest
    [PSCustomObject]@{ Algorithm = @("Ethash");               Fee = @(0.01);  MinMemGB = ($Pools."Ethash".DAGSize + 0.95GB) / 1GB;       MinerSet = 1; WarmupTimes = @(45, 75); Arguments = " --algo=ethash" } # PhoenixMiner-v6.0c is fastest
