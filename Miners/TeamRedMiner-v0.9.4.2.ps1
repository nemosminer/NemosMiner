...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("EtcHash");              Fee = @(0.01);  MinMemGB = ($Pools."EtcHash".DAGSize + $DAGmemReserve) / 1GB;      MinerSet = 1; WarmupTimes = @(45, 75); Arguments = " --algo=etchash" } # PhoenixMiner-v6.0c is fastest
    [PSCustomObject]@{ Algorithm = @("Ethash");               Fee = @(0.01);  MinMemGB = ($Pools."Ethash".DAGSize + $DAGmemReserve) / 1GB;       MinerSet = 1; WarmupTimes = @(45, 75); Arguments = " --algo=ethash" } # PhoenixMiner-v6.0c is fastest
    [PSCustomObject]@{ Algorithm = @("Ethash", "SHA256ton");  Fee = @(0.01);  MinMemGB = ($Pools."Ethash".DAGSize + $DAGmemReserve) / 1GB;       MinerSet = 0; WarmupTimes = @(45, 90); Arguments = " --algo=ethash" }
    [PSCustomObject]@{ Algorithm = @("EthashLowMem");         Fee = @(0.01);  MinMemGB = ($Pools."EthashLowMem".DAGSize + $DAGmemReserve) / 1GB; MinerSet = 1; WarmupTimes = @(45, 75); Arguments = " --algo=ethash" }
    ...
    [PSCustomObject]@{ Algorithm = @("KawPoW");               Fee = @(0.02);  MinMemGB = ($Pools."KawPoW".DAGSize + $DAGmemReserve) / 1GB;       MinerSet = 0; WarmupTimes = @(60, 75); Arguments = " --algo=kawpow" } # Wildrig-v0.31.3 is fastest on Polaris
    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices) { 
                ...
                $_.Arguments += " --user=$($Pools.($_.Algorithm[0]).User) --pass=$($Pools.($_.Algorithm[0]).Pass)$(If ($Pools.($_.Algorithm[0]).BaseName -eq "ProHashing" -and $_.Algorithm -eq "EthashLowMem") { ",l=$((($Miner_Devices.Memory | Measure-Object -Minimum).Minimum -$DAGmemReserve) / 1GB)" })"
