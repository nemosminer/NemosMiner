...
$Uri = Switch ($Variables.DriverVersion.CUDA) { 
    ...
    { $_ -ge "11.6" } { "https://github.com/sp-hash/TeamBlackMiner/releases/download/v1.86/TeamBlackMiner_1_86_cuda_11_6.7z"; Break }
    Default { Return }
}
...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithms = @("EtcHash", "VertHash"); Type = "AMD"; Fee = 0.005; MinMemGiB = $MinerPools[0].Etchash.DAGSizeGiB+ 0.81;       ExcludePools = @(@("ProHashing"), @());                 Minerset = 1; Tuning = ""; WarmupTimes = @(45, 45); Arguments = " --algo etc+vtc --verthash-data ..\.$($Variables.VerthashDatPath)" }
...
#   [PSCustomObject]@{ Algorithms = @("EthashLowMem");        Type = "AMD"; Fee = 0.005; MinMemGiB = $MinerPools[0].EthashLowMem.DAGSizeGiB + 0.81; ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = ""; WarmupTimes = @(45, 0);  Arguments = " --algo ethash" } # TTMiner-v5.0.3 is fastest
    [PSCustomObject]@{ Algorithms = @("VertHash");            Type = "AMD"; Fee = 0.01;  MinMemGiB = 2.0;                                           ExcludePools = @(@("HashCryptos", "MiningDutch"), @()); Minerset = 1; Tuning = ""; WarmupTimes = @(60, 0);  Arguments = " --algo verthash --verthash-data ..\.$($Variables.VerthashDatPath)" } # Not working in 1.82
    ...
    [PSCustomObject]@{ Algorithms = @("EtcHash", "VertHash"); Type = "INTEL"; Fee = 0.005; MinMemGiB = $MinerPools[0].Etchash.DAGSizeGiB + 0.81;      ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(45, 45); Arguments = " --algo etc+vtc --verthash-data ..\.$($Variables.VerthashDatPath)" }
...
#   [PSCustomObject]@{ Algorithms = @("EthashLowMem");        Type = "INTEL"; Fee = 0.005; MinMemGiB = $MinerPools[0].EthashLowMem.DAGSizeGiB + 0.81; ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(45, 0);  Arguments = " --algo ethash" }
    [PSCustomObject]@{ Algorithms = @("KawPow");              Type = "INTEL"; Fee = 0.01;  MinMemGiB = $MinerPools[0].KawPow.DAGSizeGiB + 0.81;       ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(60, 0);  Arguments = " --algo kawpow" }
    [PSCustomObject]@{ Algorithms = @("VertHash");            Type = "INTEL"; Fee = 0.01;  MinMemGiB = 2.0;                                           ExcludePools = @(@("HashCryptos", "MiningDutch"), @()); MinerSet = 0; Tuning = " --tweak 2"; WarmupTimes = @(60, 0);  Arguments = " --algo verthash --verthash-data ..\.$($Variables.VerthashDatPath)" }
    ...
    [PSCustomObject]@{ Algorithms = @("Etchash", "VertHash"); Type = "NVIDIA"; Fee = 0.005; MinMemGiB = $MinerPools[0].Ethash.DAGSizeGiB + 0.81;       ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(45, 45); Arguments = " --algo etc+vtc --verthash-data ..\.$($Variables.VerthashDatPath)" }
...
#   [PSCustomObject]@{ Algorithms = @("EthashLowMem");        Type = "NVIDIA"; Fee = 0.005; MinMemGiB = $MinerPools[0].EthashLowMem.DAGSizeGiB + 0.81; ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(45, 0);  Arguments = " --algo ethash" } # TTMiner-v5.0.3 is fastest
    [PSCustomObject]@{ Algorithms = @("KawPow");              Type = "NVIDIA"; Fee = 0.01;  MinMemGiB = $MinerPools[0].KawPow.DAGSizeGiB + 0.81;       ExcludePools = @(@("ProHashing"), @());                 Minerset = 2; Tuning = " --tweak 2"; WarmupTimes = @(75, 20); Arguments = " --algo kawpow" }
    [PSCustomObject]@{ Algorithms = @("VertHash");            Type = "NVIDIA"; Fee = 0.01;  MinMemGiB = 2.0;                                           ExcludePools = @(@("HashCryptos", "MiningDutch"), @()); MinerSet = 0; Tuning = " --tweak 2"; WarmupTimes = @(60, 0);  Arguments = " --algo verthash --verthash-data ..\.$($Variables.VerthashDatPath)" }
)
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGiB -ge $_.MinMemGiB) { 
                ...
                If ($_.Algorithms[1] -eq "VertHash") { 
                    ...
                    $Arguments += If ($MinerPools[1].($_.Algorithms[1]).PoolPorts[1]) { " --vtc-port $($MinerPools[1].($_.Algorithms[1]).PoolPorts[1])" } Else { " --vtc-port $($MinerPools[1].($_.Algorithms[1]).PoolPorts[0])" }
                }
