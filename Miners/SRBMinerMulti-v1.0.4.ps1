...
$Uri = "https://github.com/doktor83/SRBMiner-Multi/releases/download/1.0.4/SRBMiner-Multi-1-0-4-win64.zip"
...
#    [PSCustomObject]@{ Algorithm = @("Autolykos2");                Type = "AMD"; Fee = @(0.015);        MinMemGB = $MinerPools[0]."Autolykos2".DAGSizeGB;   MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(45, 30); ExcludePool = @()       ; Arguments = " --algorithm autolykos2" } # Hashrate far too high
...
#    [PSCustomObject]@{ Algorithm = @("EthashLowMem", "HeavyHash"); Type = "AMD"; Fee = @(0.0065, 0.01); MinMemGB = $MinerPools[0]."EthashLowMem".DAGSizeGB; MemReserveGB = 0.42; MinerSet = 1; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm ethash --algorithm heavyhash --gpu-dual-max-loss 5 --gpu-auto-tune 2" }
     [PSCustomObject]@{ Algorithm = @("FiroPoW");                   Type = "AMD"; Fee = @(0.0085);       MinMemGB = $MinerPools[0]."FiroPoW".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 0; WarmupTimes = @(90, 75); ExcludePool = @()       ; Arguments = " --algorithm firopow" }
