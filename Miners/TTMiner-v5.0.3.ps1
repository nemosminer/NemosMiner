...
#   [PSCustomObject]@{ Algorithm = "Eaglesong"; MinMemGB = 2; Command = " -algo EAGLESONG" } #Profit very small
            ...
            $Commands | ForEach-Object {
                ...
                If ($Miner_Devices = @($SelectedDevices | Where-Object { ($_.OpenCL.GlobalMemSize / 1GB) -ge $MinMemGB })) { 
                    ...
                    [PSCustomObject]@{ 
                        ...
                        Type       = "NVIDIA"
                        Path       = $Path
                        Arguments  = ("$($_.Command) --pool stratum+tcp://$($Pools.($_.Algorithm).Host):$($Pools.($_.Algorithm).Port) -work-timeout 500000 -user $($Pools.($_.Algorithm).User) -pass $($Pools.($_.Algorithm).Pass) --api-bind 127.0.0.1:$($MinerAPIPort) -device $(($Miner_Devices | ForEach-Object { '{0:x}' -f ($_.$DeviceEnumerator) }) -join ' ')" -replace "\s+", " ").trim()
