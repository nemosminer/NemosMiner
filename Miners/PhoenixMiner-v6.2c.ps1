    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices) { 
                ...
                $_.Arguments += " -pool $(If ($MinerPools[0].($_.Algorithm[0]).SSL) { "ssl://" })$($MinerPools[0].($_.Algorithm[0]).Host):$($MinerPools[0].($_.Algorithm[0]).Port) -wal $($MinerPools[0].($_.Algorithm[0]).User)"
                ...
                If ($_.Algorithm[1]) { 
                    $_.Arguments += " -dpool $(If ($MinerPools[1].($_.Algorithm[1]).SSL) { "ssl://" })$($MinerPools[1].($_.Algorithm[1]).Host):$($MinerPools[1].($_.Algorithm[1]).Port) -dwal $($MinerPools[1].($_.Algorithm[1]).User) -dpass $($MinerPools[1].($_.Algorithm[1]).Pass)"
