...
$Algorithms = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithms = @("Ethash");                  Type = "AMD"; Fee = @(0.0065);   MinMemGiB = $MinerPools[0].Ethash.DAGSizeGiB + 0.42;       MinerSet = 0; Tuning = " -mi 12 -straps 1"; WarmupTimes = @(45, 15); ExcludeGPUArchitecture = @();                  Arguments = " -amd -eres 1" } # GMiner-v3.22 is just as fast, PhoenixMiner-v6.2c is maybe faster, but I see lower speed at the pool
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices) { 
                ...
                $Arguments += " -pass $($MinerPools[0].($_.Algorithms[0]).Pass)$(If ($MinerPools[0].($_.Algorithms[0]).BaseName -eq "ProHashing" -and $_.Algorithm -eq "EthashLowMem") { ",l=$((($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum) / 1GB - ($_.MinMemGiB - $MinerPools[0].($_.Algorithm).DAGSizeGiB))" })"
                ...
                If (($AvailableMiner_Devices.Memory | Measure-Object -Minimum).Minimum / 1GB -ge 2 * $MinMemGiB -and -not $_.Algorithms[1]) { # Faster kernels require twice as much VRAM
