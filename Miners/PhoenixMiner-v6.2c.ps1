        ...
        $Algorithms | Where-Object Type -EQ $_.Type | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGiB -GE $_.MinMemGiB | Where-Object Architecture -notin $_.ExcludeGPUArchitecture) { 
                ...
                If ($AvailableMiner_Devices) { 
                    ...
                    $Miner_Name = "$($Name)-$($AvailableMiner_Devices.Count)x$($AvailableMiner_Devices.Model | Select-Object -Unique)$(If ($_.Algorithms[1]) { "-$($_.Algorithms[0])&$($_.Algorithms[1])" })$(If ($_.Intensity) { "-$($_.Intensity)" })" -replace ' '

                    $Arguments = $_.Arguments
                    $Arguments += " -pool $(If ($AllMinerPools.$_.Algorithms[0].PoolPorts[1]) { "ssl://" })$($AllMinerPools.$_.Algorithms[0].Host):$($AllMinerPools.$_.Algorithms[0].PoolPorts | Select-Object -Last 1) -wal $($AllMinerPools.$_.Algorithms[0].User)"
                    ...
                    If ($AllMinerPools.$_.Algorithms[0].PoolPorts[1]) { $Arguments += " -weakssl" }
                    ...
                    If ($AllMinerPools.$_.Algorithms[0].DAGSizeGiB -gt 0) { 
                        If ($AllMinerPools.$_.Algorithms[0].BaseName -in @("MiningPoolHub", "ProHashing")) { $Arguments += " -proto 1" }
                        ElseIf ($AllMinerPools.$_.Algorithms[0].BaseName -eq "NiceHash") { $Arguments += " -proto 4" }
                    }
                    ...
                    If ($_.Algorithms[1]) { 
                        $Arguments += " -dpool $(If ($AllMinerPools.$_.Algorithms[1].PoolPorts[1]) { "ssl://" })$($AllMinerPools.$_.Algorithms[1].Host):$($AllMinerPools.$_.Algorithms[1].PoolPorts | Select-Object -Last 1) -dwal $($AllMinerPools.$_.Algorithms[1].User) -dpass $($AllMinerPools.$_.Algorithms[1].Pass)"
                        # If ($AllMinerPools.$_.Algorithms[0].PoolPorts[1]) { $Arguments += " -dweakssl" } #https://bitcointalk.org/index.php?topic=2647654.msg60898131#msg60898131
