...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    [PSCustomObject]@{ Algorithm = "HeavyHash";     MinMemGB = 2; MinerSet = 0; WarmupTimes = @(0, 30); Arguments = " --scan-time 1 --gpu-threads 1 --worksize 256 --intensity 23 --kernel heavyhash" }
    ...
    $Devices | Where-Object Type -EQ "AMD" | Select-Object Model -Unique | ForEach-Object { 
        ...
        If ($Miner_Devices = @($Devices | Where-Object Model -EQ $_.Model)) { 
            ...
            $AlgorithmDefinitions | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
                ...
                If ($AvailableMiner_Devices = @($Miner_Devices | Where-Object { [Uint]($_.OpenCL.GlobalMemSize / 0.99GB) -ge $MinMemGB -and $_.OpenCL.ClVersion -ge "OpenCL C 2.0" })) { 
