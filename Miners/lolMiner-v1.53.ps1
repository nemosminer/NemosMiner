...
$Uri = "https://github.com/Lolliedieb/lolMiner-releases/releases/download/1.53/lolMiner_v1.53_Win64.zip"
$Name = (Get-Item $MyInvocation.MyCommand.Path).BaseName
$Path = ".\Bin\$($Name)\lolminer.exe"
$DeviceEnumerator = "Bus"

$Algorithms = [PSCustomObject[]]@(
    [PSCustomObject]@{ Algorithm = @("Autolykos2");             Type = "AMD"; Fee = @(0.015);      MinMemGB = $MinerPools[0]."Autolykos2".DAGSizeGB;   MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 45); ExcludeGPUArchitecture = @();         Arguments = " --algo AUTOLYKOS2" }
    [PSCustomObject]@{ Algorithm = @("Blake3");                 Type = "AMD"; Fee = @(0.015);      MinMemGB = 3.0;                                     MemReserveGB = 0;    MinerSet = 0; WarmupTimes = @(45, 45); ExcludeGPUArchitecture = @();         Arguments = " --algo ALEPHIUM" }
    ...
    [PSCustomObject]@{ Algorithm = @("Equihash1445");           Type = "AMD"; Fee = @(0.01);       MinMemGB = 3.0;                                     MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(30, 30); ExcludeGPUArchitecture = @();         Arguments = " --coin AUTO144_5" }
    [PSCustomObject]@{ Algorithm = @("Equihash1927");           Type = "AMD"; Fee = @(0.01);       MinMemGB = 3.0;                                     MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(45, 30); ExcludeGPUArchitecture = @();         Arguments = " --coin AUTO192_7" }
    ...
    [PSCustomObject]@{ Algorithm = @("EtcHash", "Kaspa");       Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $MinerPools[0]."Etchash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); ExcludeGPUArchitecture = @("Pascal"); Arguments = " --algo ETCHASH --dualmode KASPADUAL" }
    ...
    [PSCustomObject]@{ Algorithm = @("Ethash", "Kaspa");        Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $MinerPools[0]."Ethash".DAGSizeGB;       MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); ExcludeGPUArchitecture = @("Pascal"); Arguments = " --algo ETHASH --dualmode KASPADUAL" }
    ...
    [PSCustomObject]@{ Algorithm = @("EthashLowMem", "Kaspa");  Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $MinerPools[0]."EthashLowMem".DAGSizeGB; MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(45, 45); ExcludeGPUArchitecture = @("Pascal"); Arguments = " --algo ETHASH --dualmode KASPADUAL" }
    ...
    [PSCustomObject]@{ Algorithm = @("UbqHash", "Kaspa");       Type = "AMD"; Fee = @(0.01, 0.01); MinMemGB = $MinerPools[0]."UbqHash".DAGSizeGB;      MemReserveGB = 0.42; MinerSet = 2; WarmupTimes = @(60, 45); ExcludeGPUArchitecture = @("Pascal"); Arguments = " --algo UBQHASH --dualmode KASPADUAL" }

    [PSCustomObject]@{ Algorithm = @("Autolykos2");             Type = "NVIDIA"; Fee = @(0.015);      MinMemGB = $MinerPools[0]."Autolykos2".DAGSizeGB;   MemReserveGB = 0;    MinerSet = 1; WarmupTimes = @(45, 45); Arguments = " --algo AUTOLYKOS2" }
    ...
    $Devices | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        $Algorithms | Where-Object Type -EQ $_.Type | Select-Object | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge ($_.MinMemGB + $_.MemReserveGB) | Where-Object { $_.Architecture -notin $ExcludeGPUArchitecture }) { 
                ...
                $_.Arguments += " --tls $(If ($MinerPools[0].($_.Algorithm[0]).SSL) { "on" } Else { "off" }) --pool $($MinerPools[0].($_.Algorithm[0]).Host):$($MinerPools[0].($_.Algorithm[0]).Port)"
