...
$Algorithms = [PSCustomObject[]]@(
    ...
    # [PSCustomObject]@{ Algorithm = "YescryptR16";   MinMemGB = 2; MinerSet = 0; WarmupTimes = @(60, 0); Arguments = " --scan-time 1 --gpu-threads 1 --worksize 256 --intensity 17 --kernel yescryptr16" } # High number of bad shares
)
    ...
    $Devices | Select-Object Model -Unique | ForEach-Object { 
        ...
        $Algorithms | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
            ...
            If ($AvailableMiner_Devices = $Miner_Devices | Where-Object MemoryGB -ge $_.MinMemGB) { 
                ...
                If (-not ($Devices.Model -match "^Radeon RX [56]\d\d\d" -and $Devices.Model -notmatch "^Radeon RX [56]\d\d\d") -and $AvailableMiner_Devices.Model -match "^Radeon RX [56]\d\d\d") { $_.Arguments += "_navi" } # Current code cannot auto handle both navi and pre-navi cards (https://github.com/fancyIX/sgminer-phi2-branch/issues/251)
