...
While (-not ($APIResponse) -and $RetryCount -gt 0) { 
    Try { 
        If (-not $APIResponse) { $APIResponse = Invoke-RestMethod "http://miningpoolhub.com/index.php?page=api&action=getuserallbalances&api_key=$($Config.MiningPoolHubAPIKey)" -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop} 
    }
    Catch { }
    If (-not $APIResponse) { 
...
$APIResponse.getuserallbalances.data | Where-Object coin | Where-Object confirmed -gt 0 | ForEach-Object { 
    ...
    while (-not ($Currency) -and $RetryCount -gt 0) { 
        Try { 
            $Currency = Invoke-RestMethod "http://$($_.coin).miningpoolhub.com/index.php?page=api&action=getpoolinfo&api_key=$($Config.MiningPoolHubAPIKey)" -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop | Select-Object -ExpandProperty getpoolinfo | Select-Object -ExpandProperty data | Select-Object -ExpandProperty currency
        }
        Catch { 
            Start-Sleep -Seconds $RetryDelay # Pool might not like immediate requests
        }
