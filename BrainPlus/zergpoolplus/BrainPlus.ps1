...
Foreach ($Algo in ($AlgoData | gm -MemberType NoteProperty).Name) {
        $BasePrice = If ($AlgoData.($Algo).actual_last24h_shared) {$AlgoData.($Algo).actual_last24h_shared / 1000} else {$AlgoData.($Algo).estimate_last24h}
