...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("EtcHash");              Fee = @(0.01); MinMemGB = 3;        MinerSet = 1; WarmupTimes = @(0, 45);  Arguments = " --algo etchash --intensity 25" } # GMiner-v2.74 is fastest
    [PSCustomObject]@{ Algorithm = @("Ethash");               Fee = @(0.01); MinMemGB = 5;        MinerSet = 1; WarmupTimes = @(0, 45);  Arguments = " --algo ethash --intensity 25" } # GMiner-v2.74 is fastest
    ...
    [PSCustomObject]@{ Algorithm = @("Ethash", "FiroPoW");    Fee = @(0.01, 0.01); MinMemGB = 10; MinerSet = 0; WarmupTimes = @(0, 240); Arguments = " --algo ethash --lhr-algo firopow --lhr-tune -1" }
    [PSCustomObject]@{ Algorithm = @("Ethash", "KawPoW");     Fee = @(0.01, 0.01); MinMemGB = 10; MinerSet = 0; WarmupTimes = @(0, 240); Arguments = " --algo ethash --lhr-algo kawpow --lhr-tune -1" }
