...
$Path = ".\Bin\NVIDIA-trex0122b\t-rex.exe"
...
$Commands | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
    ...
    [PSCustomObject]@{
        Type      = "NVIDIA"
        Path      = $Path
        Arguments = "--no-watchdog --no-nvml -b 127.0.0.1:$($Variables.NVIDIAMinerAPITCPPort) -d $($Config.SelGPUCC) -o stratum+tcp://$($Pools.($Algo).Host):$($Pools.($Algo).Port) -u $($Pools.($Algo).User) -p $($Pools.($Algo).Pass)$($Commands.$_) --quiet -R 1 -T 50000 --cpu-priority 4"
