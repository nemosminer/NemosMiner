...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "EtcHash";    Fee = 0.01; MinMemGB = 4; MinerSet = 1; Arguments = " --algo etchash --intensity 25" } # GMiner-v2.44 is fastest
    [PSCustomObject]@{ Algorithm = "Ethash";     Fee = 0.01; MinMemGB = 4; MinerSet = 1; Arguments = " --algo ethash --intensity 25" } # GMiner-v2.44 is fastest
    ...
    [PSCustomObject]@{ Algorithm = "X21s";       Fee = 0.01; MinMemGB = 3; MinerSet = 0; Arguments = " --algo x21s --intensity 23" }
    [PSCustomObject]@{ Algorithm = "X25x";       Fee = 0.01; MinMemGB = 3; MinerSet = 0; Arguments = " --algo x25x --intensity 21" }
    ...
    $Devices | Where-Object Type -EQ "NVIDIA" | Select-Object Model -Unique | ForEach-Object { 
        ...
        If ($SelectedDevices = @($Devices | Where-Object Model -EQ $_.Model)) { 
            ...
            $AlgorithmDefinitions | ForEach-Object {
                ...
                ElseIf ($_.Algorithm -eq "MTP") { $WaitForData = 15 } # Seconds, max. wait time until first data sample
                ElseIf ($_.Algorithm -match "^Octopus$|^Sonoa$") { $WaitForData = 30 } # Seconds, max. wait time until first data sample
