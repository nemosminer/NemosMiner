...
$Path = ".\Bin\NVIDIA-nanominer152\cmdline_launcher.bat"
...
$Commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object { 
    ...
    [PSCustomObject]@{
        Type      = "NVIDIA"
        Path      = $Path
        Arguments = "-devices $($Config.SelGPUCC) -mport -$($Variables.NVIDIAMinerAPITCPPort) -algo $($Commands.$_) -wallet $($Pools.($Algo).User) -rigName $($Pools.($Algo).Pass) -pool1 $($Pools.($Algo).Host):$($Pools.($Algo).Port)"
