...
$Uri = "https://github.com/todxx/teamredminer/releases/download/v0.8.6/teamredminer-v0.8.6-win.zip"
...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "FiroPoW";              Fee = 0.02;  MinMemGB = 3.0; MinerSet = 0; WarmupTimes = @(60, 60); Arguments = " --algo=firopow" } # Wildrig-v0.29.1 is fastest on Polaris
    ...
    $Devices | Where-Object Type -EQ "AMD" | Select-Object Model -Unique | ForEach-Object { 
        ...
        If ($Miner_Devices = @($Devices | Where-Object Model -EQ $_.Model)) { 
            ...
            $AlgorithmDefinitions | ConvertTo-Json | ConvertFrom-Json | ForEach-Object {
                ...
                $AvailableMiner_Devices = @($Miner_Devices | Where-Object { ($_.OpenCL.GlobalMemSize / 1GB) -ge $MinMemGB -and $_.OpenCL.ClVersion -ge "OpenCL C 2.0" })
                ...
                If ($AvailableMiner_Devices) {
                    ...
                    If ($Pools.($_.Algorithm).DAGsize -ne $null -and $Pools.($_.Algorithm).Name -match "^NiceHash$|^MiningPoolHub(|Coins)$") { $_.Arguments += " --eth_stratum_mode=nicehash" }
                    ...
                    [PSCustomObject]@{ 
                        ...
                        Type        = "AMD"
                        Path        = $Path
                        Arguments   = ("$($_.Arguments) --url $($Protocol)://$($Pools.($_.Algorithm).Host):$($Pools.($_.Algorithm).Port) --user $($Pools.($_.Algorithm).User)$Pass--watchdog_disabled --no_gpu_monitor --init_style=3 --platform $($AvailableMiner_Devices.PlatformId | Sort-Object -Unique) --api_listen=127.0.0.1:$MinerAPIPort --devices $(($AvailableMiner_Devices | Sort-Object $DeviceEnumerator -Unique | ForEach-Object { '{0:d}' -f $_.$DeviceEnumerator }) -join ',')" -replace "\s+", " ").trim()
