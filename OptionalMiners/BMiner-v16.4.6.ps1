...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("Equihash1445");  Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 3.0; MinerSet = 1; WarmupTimes = @(0, 30); Protocol = @(" -pers auto -uri equihash1445") } # MiniZ-v1.6x is fastest
    [PSCustomObject]@{ Algorithm = @("EquihashBTG");   Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 3.0; MinerSet = 1; WarmupTimes = @(0, 30); Protocol = @(" -uri zhash") }
    ...
    [PSCustomObject]@{ Algorithm = @("Octopus");       Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 3.0; MinerSet = 1; WarmupTimes = @(0, 30); Protocol = @(" -uri conflux") } # NBMiner-v39.5 is faster is faster but has 2% fee
    ...
    $Devices | Where-Object Type -in @("AMD", "NVIDIA") | Select-Object Type, Model -Unique | ForEach-Object { 
        ...
        If ($SelectedDevices = @($Devices | Where-Object Type -EQ $_.Type | Where-Object Model -EQ $_.Model)) { 
            ...
            $AlgorithmDefinitions | Where-Object Type -EQ $_.Type | ForEach-Object { 
                ...
                If ($_.Algorithm[0] -match "^Ethash.*$") { $Miner_Devices = @($Miner_Devices | Where-Object { $_.Model -notmatch "^Radeon RX 5[0-9]{3}.*" }) } # Ethash mining not supported on Navi
                If ($_.Algorithm[1]) { $Miner_Devices = @($Miner_Devices | Where-Object { $_.Model -notmatch "^Radeon RX 5[0-9]{3}.*" }) } # Dual mining not supported on Navi
