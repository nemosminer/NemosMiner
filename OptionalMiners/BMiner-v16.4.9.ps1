...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = @("EquihashBTG");   Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 3.0; MinerSet = 1; WarmupTimes = @(20, 30); Protocol = @(" -uri zhash") }
    ...
    [PSCustomObject]@{ Algorithm = @("Octopus");       Type = "NVIDIA"; Fee = @(0.02);   MinMemGB = 3.0; MinerSet = 1; WarmupTimes = @(20, 30); Protocol = @(" -uri conflux") } # NBMiner-v39.5 is faster is faster but has 2% fee
            ...
            $AlgorithmDefinitions | Where-Object Type -EQ $_.Type | ConvertTo-Json | ConvertFrom-Json | ForEach-Object { 
                ...
                If ($AvailableMiner_Devices) { 
                    ...
                    [PSCustomObject]@{ 
                        ...
                        Type        = $_.Type
                        Path        = $Path
                        Arguments   = ("$Arguments -max-network-failures=0 -watchdog=false -api 127.0.0.1:$($MinerAPIPort) -devices $(($AvailableMiner_Devices | Sort-Object $DeviceEnumerator | ForEach-Object { "$(If ($AvailableMiner_Devices.Vendor -eq "AMD") { "amd:" }){0:x}" -f $_.$DeviceEnumerator }) -join ',')" -replace "\s+", " ").trim()
