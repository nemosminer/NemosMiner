...
$Commands = [PSCustomObject]@{ 
    ...
    "eaglesong+ethash" = "-a eaglesong_ethash --fee 1 --no-watchdog -di 24,24,24,24,24,24,24,24,24 -o stratum+tcp://" #eaglesong + ethash
    "handshake+ethash" = "-a hns_ethash --fee 1 --no-watchdog -di 4,4,4,4,4,4,4,4,4 -o stratum+tcp://" #handshake + ethash
...
$Commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object { $Algo = Get-Algorithm ($_ -split '\+' | Select-Object -Index 0); $Algo2 = Get-Algorithm ($_ -split '\+' | Select-Object -Index 1); $_ } | Where-Object { $Pools.$Algo.Host } | ForEach-Object { 
    switch ($_) { 
        "ethash" { $Fee = 0.0065 }
        "eaglesong_ethash" { $Fee = 0.03, 0.03 }
        default { $Fee = 0.01 }
    }
