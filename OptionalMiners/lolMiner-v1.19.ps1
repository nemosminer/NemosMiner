...
$Uri = "https://github.com/Lolliedieb/lolMiner-releases/releases/download/1.19/lolMiner_v1.19_Win64.zip"
...
$AlgorithmDefinitions = [PSCustomObject[]]@(
    ...
    [PSCustomObject]@{ Algorithm = "CuckarooD29";   Type = "AMD"; Fee = 0.01;  MinMemGB = 4.0; MinerSet = 1; Arguments = " --algo C29D" } # TeamRed-v0.8.0 is fastest, keep enabled because TeamRed does not support algo on Navi
    [PSCustomObject]@{ Algorithm = "CuckarooM29";   Type = "AMD"; Fee = 0.01;  MinMemGB = 6.0; MinerSet = 0; Arguments = " --algo C29M" }
    [PSCustomObject]@{ Algorithm = "Cuckatoo31";    Type = "AMD"; Fee = 0.01;  MinMemGB = 4.0; MinerSet = 1; Arguments = " --algo C31" } # TeamRed-v0.8.0 is fastest
    ...
    [PSCustomObject]@{ Algorithm = "Equihash1445";  Type = "NVIDIA"; Fee = 0.01;  MinMemGB = 2.0; MinerSet = 1; Arguments = " --coin AUTO144_5" } # MiniZ-v1.6x is fastest, but has 2% miner fee
            ...
            $AlgorithmDefinitions | Where-Object Type -EQ $_.Type | Where-Object { -not $Pools.($_.Algorithm).SSL } | ForEach-Object {
                ...
                If ($Miner_Devices = @($SelectedDevices | Where-Object { ($_.OpenCL.GlobalMemSize / 1GB) -ge $MinMemGB })) { 
                    ...
                    [PSCustomObject]@{ 
                        ...
                        Type            = $_.Type
                        Path            = $Path
                        Arguments       = ("$Arguments --pool $($Pools.($_.Algorithm).Host):$($Pools.($_.Algorithm).Port) --user $($Pools.($_.Algorithm).User) --pass $($Pools.($_.Algorithm).pass)$(If ($Pools.($_.Algorithm).SSL) { " --tls on" } Else { " --tls off" } ) --log off --apiport $MinerAPIPort --devicesbypcie --shortstats=2 --longstats=10 --devices $(($Miner_Devices | Sort-Object $DeviceEnumerator | ForEach-Object { '{0}:0' -f $_.$DeviceEnumerator }) -join ',')").trim()
